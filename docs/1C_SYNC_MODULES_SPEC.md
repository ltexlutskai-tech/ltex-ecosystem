...
### 3.6 СтворитиМаршрутнийЛист (M1.9 Маршрутний лист, Етап 5 — implemented client-side, реальний BSL чекає 1С-розробника)

Створює / оновлює документ `Document.МаршрутныйЛист` у central 1С на основі
одного мобільного маршрутного листа (наш `RouteSheet` + дочірні таблиці). На
відміну від решти операцій, **МЛ — документ-агрегатор дня виїзду**, тому контракт
обміну **двофазний** (аудит §H, `Central/CommonModules/ОбменАРМ/Ext/Module.bsl`):

> **Вивантаж (central → mobile)** — `ДобавитьМаршрутныйЛист` (`:2632`): не один
> UNION, а **2 запити + обчислення** — Запит #1 `Заказы × ТоварыЗаказов`
> (`:2636`, з `КоличествоЗагружено`), Запит #2 `ЗагрузкаМашины` (`:2715`, зі
> `Штрихкод`/`Вес`/`ЦенаПродажиВес`), + обчислена `НеХватает`
> (`ПолучитьТаблицуНеХватаетМаршрутныйЛист`, `:2806`, проти
> `РегистрНакопления.ТоварыНаСклади`). Шапка несе `ИД`/`Ссылка`/`Дата`/
> `СтатусДокумента`(рядок)/`Комментарий`/`ПометкаУдаления`; **`Номер`/
> `ДатаПриезда`/`СуммаДокумента` у вивантаж НЕ кладуться** (хоч і вибираються).
> `Заказы[]` — без `Город` (мобільна деривація з `Контрагент.Город`).
>
> **Завантаж (mobile → central)** — `ОбработатьПакетДанных` (`:3199`) обробляє МЛ
> **двічі**:
>
> - **Фаза A** `СоздатьМаршрутныеЛистыТовары` (`:5283`) — шапка + таб. частини
>   `Заказы` / `ТоварыЗаказов` / `Завдання` / `ЗагрузкаМашини`. Рядки плоскі з
>   дискримінатором **`ТабЧасть`** (`"Заказы"` / `"ТоварыЗаказов"` / `"Завдання"` /
>   `"ЗагрузкаМашини"` / `"Продажи"`). `ДатаПриезда` **приймається** на вході
>   (`:5311`). Статус **рядок→enum** через тернар (`:5312`).
> - **Фаза B** `СоздатьМаршрутныеЛистыДокументы` (`:5466`, **після** створення
>   Реалізацій/ПКО) — таб. частина `Продажи` (`ТабЧасть="Продажи"`) + `Оплаты`.
>   **Немає `ТабЧасть="Реализации"`/`"Оплаты"`**: Реалізації приходять окремим
>   верхнім ключем `Реализации`, а **Оплати відновлюються на сервері** з
>   пов'язаних ПКО/РКО (`:5582`), не з окремої таб. частини.
>
> **«Замок після завершення»** — `ВозвратПоЗакритомуМаршрутнику` (`:3993`): якщо
> МЛ у central має статус `Завершен`, вхідний об'єкт **пропускається** (ID
> підтверджується, щоб мобільний перестав надсилати). Це реальний lock, **а не
> `Архив`** (у нас `status === "completed"`). `РеализацияТоваровУслуг.МаршрутныйЛист`
> (DocRef) пишеться на вході (`:3718`), читається у вивантажі.
> `КонтрагентыСАктуальнымиЗаказами.ЗаказВключенВМаршрутныйЛист` (bool) — central
> обчислює LEFT JOIN на `МаршрутныйЛист.Заказы` (`:6006`). Кілометраж + GPS
> їдуть **у тому ж пакеті** (`Обмен_АнализДанних`: `Кілометраж` за 7 днів,
> `Координаты` за сьогодні) — у нас вони на самій шапці МЛ
> (`mileageStartKm`/`mileageEndKm`/`gpsLat`/`gpsLng`).

> **Транспорт замокано.** На нашій стороні (`apps/store`) маршрутний лист
> ставиться у чергу `mgr_sync_jobs` (entityType `route_sheet`) **лише на переході
> статусу у `dispatched` (Отправлен) або `completed` (Завершен)** — створення
> чернетки НЕ синкається (значущий момент — відправка у виїзд / завершення дня).
> `services/manager-sync` проксі за замовчуванням працює у `SYNC_MOCK_MODE`
> (синтетична відповідь `MOCK-RSH-…` через `createRouteSheetMock`). Реальний
> SOAP-виклик (`createRouteSheetViaSoap`) активується коли виставлено
> `ONEC_SOAP_URL`/`ONEC_SOAP_PASSWORD` і реалізовано BSL нижче.
...
| `ПарольВхода`    | xs:string | In     | Shared secret          |
| `IdempotencyKey` | xs:string | In     | UUID для dedup         |
| `ПакетДанних`    | xs:string | In     | JSON-payload МЛ (граф) |

**`ПакетДанних` JSON структура** (точно такий shape що шле наша
`apps/store/lib/sync/enqueue.ts::buildRouteSheetCreatePayload`):

```json
{
  "routeSheetInternalId": "ckrs123abc",
  "code1C": null,
  "docNumber": 7,
  "date": "2026-05-25T08:00:00.000Z",
  "arrivalDate": "2026-05-25T09:30:00.000Z",
  "status": "dispatched",
  "routeCode1C": "RT-001",
  ...
  "gpsLng": "25.325000",
  "orders": [
    { "orderCode1C": "ORD-7", "customerCode1C": "000001", "city": "Луцьк" }
  ],
  "items": [
    {
      "orderCode1C": "ORD-7",
...
**Поля → бізнес-ключі обміну + таб. частини (§H):**

- `routeSheetInternalId` — наш `RouteSheet.id` (cuid). 1С зберігає у `ВнішнійID`
  документа для dedup при retry-storms.
- `code1C` — null коли наша сторона ще не знає 1С-кода (звичайний create). 1С
  повертає свій код (`Ref`/GUID) у response.
- `docNumber` — інформативний (`Номер`); central сам генерує номер
  (`Номер/ДатаПриезда/Сумма` у вивантаж не йдуть).
- `status` — `draft`/`dispatched`/`completed` → enum
  `СтатусыМаршрутногоЛиста.Составляется`/`Отправлен`/`Завершен` (рядок→enum
  тернаром, `:5312`). **Якщо у central МЛ уже `Завершен`** → спрацьовує
  `ВозвратПоЗакритомуМаршрутнику`: об'єкт пропускається, лише ID підтверджується.
- `routeCode1C` / `expeditorCode1C` — наші додаткові поля шапки (1С їх на МЛ не
  тримає; маршрут — порожній довідник, експедитор — у регістрах). Передаємо для
  майбутньої прив'язки; central може ігнорувати, поки не доданий реквізит.
- `arrivalDate` (`ДатаПриезда`) — **приймається** на вході Фази A (`:5311`).
- `mileageStartKm`/`mileageEndKm` (км, .1f) + `gpsLat`/`gpsLng` (.6f) —
  кілометраж/координати. У central їдуть тим самим пакетом
  (`Обмен_АнализДанних`); **⚠️ центральний імпорт — «сирота»** (writers
  `ЗаписатьКілометражТорговогоАгента`/`…Координаты` без викликача,
  `ОбработатьПакетДанных` ці ключі не диспетчить) — поки приймаємо best-effort.
- `orders[]` → таб. частина **`Заказы`** (`ТабЧасть="Заказы"`): `orderCode1C`
  (`ЗаказПокупателя` через `Documents.Заказ.НайтиПоКоду`/`УИД`), `customerCode1C`
  (`Контрагент`). `city` — мобільна деривація (central НЕ приймає як вхід).
- `items[]` → таб. частина **`ТоварыЗаказов`** (`ТабЧасть="ТоварыЗаказов"`):
  `productCode1C` (`Номенклатура`), `lotBarcode` (`ХарактеристикаНоменклатуры`/
  лот за ШК), `quantity` (`Количество`), `quantityLoaded` (`КоличествоЗагружено`),
  `price`/`sum`.
- `loading[]` → таб. частина **`ЗагрузкаМашини`** (`ТабЧасть="ЗагрузкаМашини"`):
  `lotBarcode` (`Штрихкод`), `weight` (`Вес`), `pricePerKg` (`ЦенаПродажиВес`),
  `loaded`/`isReturn` (повернення по закритому маршрутнику).
- `sales[]` → **верхній ключ `Реализации`** (НЕ таб. частина): кожен `saleCode1C`
  — реалізація, що приходить **окремо** (operation `СтворитиРеалізацію`, §3.4);
  тут лише back-ref `orderCode1C`/`customerCode1C`/`sum` для Фази B
  (`РеализацияТоваровУслуг.МаршрутныйЛист`).
- `payments[]` — оплати **відновлюються на сервері з пов'язаних ПКО/РКО**
  (`:5582`, operation `СоздатьПКО`, §3.5), **немає окремої таб. частини**; тут
  лише back-ref `cashOrderCode1C`/`saleCode1C`/`type`/`amount`.
- `tasks[]` → таб. частина **`Завдання`** (`ТабЧасть="Завдання"`):
  `customerCode1C` (`Контрагент` з менеджерського довідника) + `comment`.
- **`НеХватает` (Бракує)** — у payload НЕ передається: 1С **обчислює сам**
  (замовлено − складський залишок) і повертає у `ИнфоМаршруты` (response).
...
{
  "ok": true,
  "routeSheetCode1C": "0000007",
  "routeSheetNumber": "ML000000007",
  "errors": []
}
```
...
```json
{
  "ok": false,
  "errorCode": 2,
  "errorMessage": "Не знайдено замовлення ORD-7 у 1С"
}
```
...
- `1` — невірний пароль
- `2` — referenced entity не знайдена (замовлення/клієнт/товар/лот)
- `3` — business rule violation (напр. lock `Завершен`)
- `4` — інша помилка БД у 1С

**Чорновик BSL (для майбутньої реалізації):**

```bsl
Функція СтворитиМаршрутнийЛист(ПарольВхода, IdempotencyKey, ПакетДанних) Експорт
    Якщо Не ПеревіритиПароль(ПарольВхода) Тоді
        Возврат СтворитиВідповідьПомилки(1, "Bad password");
    КонецЕсли;
    ...
    Спробувати
        ...
        // Знайти існуючий МЛ (за code1C/ВнішнійID) або створити новий
        МЛ = ЗнайтиМаршрутнийЛист(Дані.code1C, Дані.routeSheetInternalId);

        // «Замок після завершення» — ВозвратПоЗакритомуМаршрутнику (:3993)
        Якщо МЛ <> Неопределено
            І МЛ.СтатусДокумента = Перечисления.СтатусыМаршрутногоЛиста.Завершен Тоді
            // об'єкт пропускається; ID підтверджується щоб мобільний перестав слати
            Результат = СтворитиВідповідьМаршрутнийЛист(МЛ.Код, МЛ.Номер);
            ЗберегтиСинкЛогЗапис(IdempotencyKey, "СтворитиМаршрутнийЛист", Результат);
            Возврат Результат;
        КонецЕсли;

        Якщо МЛ = Неопределено Тоді
            Об'єкт = Documents.МаршрутныйЛист.СоздатьДокумент();
            Об'єкт.Дата = ?(Дані.date <> Неопределено, ДатаИзЛокальногоISO(Дані.date), ТекущаяДата());
        Інакше
            Об'єкт = МЛ.ПолучитьОбъект();
        КонецЕсли;

        // Шапка
        Об'єкт.ДатаПриезда   = ?(Дані.arrivalDate <> Неопределено, ДатаИзЛокальногоISO(Дані.arrivalDate), Дата(1,1,1)); // :5311
        Об'єкт.СтатусДокумента = ?(Дані.status = "completed", Перечисления.СтатусыМаршрутногоЛиста.Завершен,
            ?(Дані.status = "dispatched", Перечисления.СтатусыМаршрутногоЛиста.Отправлен,
                Перечисления.СтатусыМаршрутногоЛиста.Составляется)); // рядок→enum, :5312
        Об'єкт.Комментарий = ?(Дані.comment <> Неопределено, Дані.comment, "");
        Об'єкт.ВнішнійID   = Дані.routeSheetInternalId;

        // ─── Фаза A: товарні таб. частини (Заказы/ТоварыЗаказов/Завдання/ЗагрузкаМашини) ───
        Об'єкт.Заказы.Очистить();
        Для Каждого Р Из Дані.orders Цикл
            СтрЗак = Об'єкт.Заказы.Добавить();
            СтрЗак.ЗаказПокупателя = ЗнайтиЗаказ(Р.orderCode1C);
            СтрЗак.Контрагент      = Catalogs.Контрагенты.НайтиПоКоду(Р.customerCode1C);
            // Р.city — мобільна деривація, у central не пишемо
        КонецЦикла;

        Об'єкт.ТоварыЗаказов.Очистить();
        Для Каждого Р Из Дані.items Цикл
            СтрТов = Об'єкт.ТоварыЗаказов.Добавить();
            СтрТов.ЗаказПокупателя      = ЗнайтиЗаказ(Р.orderCode1C);
            СтрТов.Контрагент           = Catalogs.Контрагенты.НайтиПоКоду(Р.customerCode1C);
            СтрТов.Номенклатура         = Catalogs.Номенклатура.НайтиПоКоду(Р.productCode1C);
            СтрТов.Характеристика       = ЗнайтиЛотЗаШтрихкодом(Р.lotBarcode);
            СтрТов.Количество           = Число(Р.quantity);
            СтрТов.КоличествоЗагружено  = Число(Р.quantityLoaded);
            СтрТов.Цена                 = Число(Р.price);
            СтрТов.Сумма                = Число(Р.sum);
        КонецЦикла;

        Об'єкт.ЗагрузкаМашины.Очистить();
        Для Каждого Р Из Дані.loading Цикл
            СтрЗагр = Об'єкт.ЗагрузкаМашины.Добавить();
            СтрЗагр.Номенклатура    = Catalogs.Номенклатура.НайтиПоКоду(Р.productCode1C);
            СтрЗагр.Характеристика  = ЗнайтиЛотЗаШтрихкодом(Р.lotBarcode);
            СтрЗагр.Штрихкод        = Р.lotBarcode;
            СтрЗагр.Вес             = Число(Р.weight);
            СтрЗагр.ЦенаПродажиВес  = Число(Р.pricePerKg);
            СтрЗагр.Загружено       = Р.loaded;
            СтрЗагр.Возврат         = Р.isReturn;
        КонецЦикла;

        Об'єкт.Завдання.Очистить();
        Для Каждого Р Из Дані.tasks Цикл
            СтрЗ = Об'єкт.Завдання.Добавить();
            СтрЗ.Контрагент = Catalogs.Контрагенты.НайтиПоКоду(Р.customerCode1C);
            СтрЗ.Комментарий = Р.comment;
        КонецЦикла;

        Об'єкт.Записать(РежимЗаписиДокумента.Проведение); // СоздатьМаршрутныеЛистыТовары, :5283

        // ─── Фаза B: Продажи + Оплати (після створення Реалізацій/ПКО) ───
        // Реалізації приходять окремо (СтворитиРеалізацію §3.4); тут лише back-ref.
        Для Каждого Р Из Дані.sales Цикл
            Реализация = ЗнайтиРеалізацію(Р.saleCode1C);
            Якщо Реализация <> Неопределено Тоді
                Об'єктР = Реализация.ПолучитьОбъект();
                Об'єктР.МаршрутныйЛист = Об'єкт.Ссылка; // DocRef, :3718
                Об'єктР.Записать();
                ДодатиРядокПродажи(Об'єкт, Реализация); // ТабЧасть="Продажи"
            КонецЕсли;
        КонецЦикла;
        // Оплати — НЕ окрема таб. частина: відновлюються з пов'язаних ПКО/РКО (:5582)
        ВідновитиОплатиЗПКО(Об'єкт, Дані.payments);

        // НеХватает — обчислюємо самі (замовлено − залишок) для ИнфоМаршруты
        ИнфоМаршрут = ПолучитьТаблицуНеХватаетМаршрутныйЛист(Об'єкт.Ссылка); // :2806

        Результат = СтворитиВідповідьМаршрутнийЛист(Об'єкт.Код, Об'єкт.Номер);
        ЗберегтиСинкЛогЗапис(IdempotencyKey, "СтворитиМаршрутнийЛист", Результат);
        Возврат Результат;
    Виключення
        Возврат СтворитиВідповідьПомилки(4, "DB write: " + ОписаниеОшибки());
...
Функція СтворитиВідповідьМаршрутнийЛист(Code, Number)
    Шаблон = "{""ok"":true,""routeSheetCode1C"":""%1"",""routeSheetNumber"":""%2"",""errors"":[]}";
    Возврат СтрШаблон(Шаблон, Code, Number);
КонецФункції
```

### 3.7 ОтриматиСнапшот (M1.6, заплановано — inbound)
