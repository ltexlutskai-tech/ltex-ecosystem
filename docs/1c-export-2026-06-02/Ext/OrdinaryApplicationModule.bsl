...
Процедура ПриНачалеРаботыСистемы()
	...
	РабочееМесто = Обработки.РабочееМесто.Создать();
	ФормаРабочегоМеста = РабочееМесто.ПолучитьФорму("Форма");
	ФормаРабочегоМеста.Открыть();
	
	//++ <Томашевський А. В., Intrata, 19.02.2022, JIRA LS-186>
	Если ПараметрыСеанса.ТекущийПользователь.УчастьВЧаті тогда
		Чат = Обработки.Чат1с.ПолучитьФорму("Форма");
		чат.СостояниеОкна = ВариантСостоянияОкна.Прячущееся ; 
		Чат.Открыть(); 
	КонецЕсли;	
	//++
...
// Подключить актуальный менеджер FREDO-Звит
//
Функция глПодключитьМенеджерЗвит1С(ВыводитьСообщенияОбОшибках = Истина) Экспорт
	...
	Иначе
		...
		Если ПравоДоступаКОтчету = Ложь ИЛИ ПравоДоступаКОтчету = Неопределено Тогда
			...
			Если ВыводитьСообщенияОбОшибках Тогда
				Предупреждение(НСтр("ru='Недостаточно прав на использование модуля взаимодействия с ""FREDO Звит""!';uk='Недостатньо прав на використання модуля взаємодії зі ""FREDO Звіт""!'"));
	...
	Если МенеджерЗвит1С = Неопределено Тогда
		Если ВыводитьСообщенияОбОшибках Тогда
			Сообщить(НСтр("ru='Не удалось получить менеджер ""FREDO Звит""!';uk='Не вдалося отримати менеджер ""FREDO Звіт""!'"), СтатусСообщения.Важное);
			...
			Если НайденныйЭлемент = Справочники.РегламентированныеОтчеты.ПустаяСсылка() Тогда
				Сообщить(НСтр("ru='В справочнике ""Регламентированные отчеты"" не найден менеджер по работе с системой ""FREDO Звит""';uk='У довіднику ""Регламентовані звіти"" не знайдено менеджер по роботі з системою ""FREDO Звіт""'"));
			КонецЕсли;
	...
	Иначе
		...
		Исключение
			...
			// Если остался устаревший менеджер FREDO-Звит. То в нем нет метода Переинициализация(...)
			Если ВыводитьСообщенияОбОшибках Тогда 
				Сообщить(НСтр("ru = 'Не удалось переподключить менеджер ""FREDO Звит"". 
                                     |Необходимо обновить менеджер FREDO Звит в справочнике ""Регламентированные отчеты""'; uk = 'Не вдалося перепідключити менеджер ""FREDO Звіт"".
