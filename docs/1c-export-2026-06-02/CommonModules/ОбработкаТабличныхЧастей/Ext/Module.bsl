	...
	Для Каждого СтрокаТовара ИЗ ТаблицаТоваров Цикл
		Если СтрокаТовара.ПроцентАвтоматическихСкидок <> 0 Тогда
			...
			Если УчитыватьНДС И СуммаВключаетНДС Тогда 
				МинимальнаяЦена = Окр(МинимальнаяЦена * (1 + (НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТовара.СтавкаНДС) / 100)), 2);
			КонецЕсли;
		...
		Если УчитыватьНДС Тогда // рассчитаем сумму НДС строки таблицы значений
			СтрокаТовара.СуммаНДС = НалоговыйУчет.РассчитатьСуммуНДС(СтрокаТовара.Сумма, УчитыватьНДС, СуммаВключаетНДС,
			   НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТовара.СтавкаНДС));
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьЦенуПродажиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	Если ЗначениеЗаполнено(Цена) Тогда
		СтрокаТабличнойЧасти.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
					...
					НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС),0));
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			СтрокаТабличнойЧасти.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
			                            ...
			                            ?(ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента, ИмяТабличнойЧасти), НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС),0));
		КонецЕсли;
...
Процедура РассчитатьРозничнуюЦенуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета, ПересчитыватьЦенуВЕдиницуХранения = Истина) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("СуммаВключаетНДС", МетаданныеДокумента) Тогда
		...
		СтавкаНДС        = НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС);
	Иначе
		СуммаВключаетНДС = Истина;
		СтавкаНДС        = НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.Номенклатура.СтавкаНДС);
	КонецЕсли;
...
Процедура РассчитатьСуммуНДСПередачиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	СтрокаТабличнойЧасти.СуммаНДСПередачи = НалоговыйУчет.РассчитатьСуммуНДС(СтрокаТабличнойЧасти.СуммаПередачи,
	...
	Если (ДокументОбъект.УчитыватьНДС) 
	   И (НЕ ДокументОбъект.СуммаВключаетНДС) Тогда
		...
		Если СуммаПлановойСебестоимости <> 0 Тогда
			...
			СуммаНДСПлановойСебестоимости = НалоговыйУчет.РассчитатьСуммуНДС(СуммаПлановойСебестоимости,
			                                ДокументОбъект.УчитыватьНДС,
			                                ДокументОбъект.СуммаВключаетНДС,
			                                НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС));
...
Процедура ПересчитатьНДСсУчетомПогрешностиОкругления(ТабличнаяЧасть, ДокументОбъект, ПогрешностиОкругления, ПредставлениеТабличнойЧасти = "", ИмяРеквизитаСумма    = "Сумма", ИмяРеквизитаСуммаНДС = "СуммаНДС", ДополнениеКТексту ="") Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		СтрокаТаблицы[ИмяРеквизитаСуммаНДС] = НалоговыйУчет.РассчитатьСуммуНДСсУчетомПогрешности(СтрокаТаблицы[ИмяРеквизитаСумма],
	                                                   ДокументОбъект.УчитыватьНДС,
	                                                   ДокументОбъект.СуммаВключаетНДС,
...
Процедура РассчитатьСуммуНДСиАкцизногоНалогаТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("Сумма", МетаданныеДокумента, ИмяТабличнойЧасти) И
		...
		КонецЕсли;
		СтрокаТабличнойЧасти.СуммаНДС = НалоговыйУчет.РассчитатьСуммуНДС(СуммаДляРасчетаНДС,
	    	                                               УчитыватьНДС, СуммаВключаетНДС,
	            	                                       НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС));
