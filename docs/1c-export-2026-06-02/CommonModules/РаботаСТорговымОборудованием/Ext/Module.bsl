	...
	Иначе
		Если ТипШтрихкода = ПланыВидовХарактеристик.ТипыШтрихкодов.EAN13 Тогда
			Если ДлинаКода = 13 Тогда 
				Если НЕ(ТолькоЦифры(Штрихкод)) // штрихкод должен состоять из цифр
				 	Или КонтрольныйСимволEAN(Лев(Штрихкод,12), 13) <> Прав(Штрихкод, 1) Тогда
						Возврат Ложь;
				КонецЕсли;
			ИначеЕсли ДлинаКода = 8 ИЛИ ДлинаКода = 26 Тогда
				Возврат Истина;		
			КонецЕсли;
...
//
Функция СформироватьШтрихКод(Знач ПрефиксШтучногоТовара = Неопределено, Знач ПрефиксВнутреннегоШтрихкода = Неопределено, ХарактеристикаНоменклатуры = Неопределено) Экспорт
	...
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	МАКСИМУМ(ПОДСТРОКА(РегШтрихКоды.Штрихкод, 8, 5)) КАК Код
	|ИЗ
	...
	Если ХарактеристикаНоменклатуры <> Неопределено Тогда 
		ВесДляШтрихкода = ВесовойУчет.ВесДляШтрихкода(ХарактеристикаНоменклатуры);
	КонецЕсли;	
	
	Штрихкод = "2" + ПрефиксШтучногоТовара + ПрефиксВнутреннегоШтрихкода + ВесДляШтрихкода + Формат(ТекКод, "ЧЦ=5; ЧВН=; ЧГ=");
...
//       – сформированный штрихкод
//
Функция СформироватьШтрихКодДокумента(Знач ПрефиксВнутреннегоШтрихкода = Неопределено) Экспорт
	...
	|ГДЕ
	|	РегШтрихКоды.ТипШтрихкода = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ТипыШтрихкодов.EAN13)
	|	И РегШтрихКоды.Штрихкод ПОДОБНО ""2" + ПрефиксДокумента + ПрефиксВнутреннегоШтрихкода + "_________""
	|");
	...
	Штрихкод = "2" + ПрефиксДокумента + ПрефиксВнутреннегоШтрихкода + Формат(ТекКод, "ЧЦ=8; ЧВН=; ЧГ=");
	Штрихкод = Штрихкод + КонтрольныйСимволEAN(ШтрихКод, 13);
