...
Процедура ЗаполнитьРасходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ПлатежноеТребованиеПоручениеВыставленное")
...
Процедура ЗаполнитьПриходПоОснованию(ДокументОбъект, Основание, ПользовательДокумента) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДоговорКонтрагента", ДокументОбъект.Метаданные()) И ЗначениеЗаполнено(ДокументОбъект.ДоговорКонтрагента) И ДокументОбъект.ДоговорКонтрагента.Внешнеэкономический Тогда
		ДокументОбъект.ВалютаДокумента = ДокументОбъект.ДоговорКонтрагента.ВалютаВзаиморасчетов;
	Иначе 
		ДокументОбъект.ВалютаДокумента = мВалютаРегламентированногоУчета;
	КонецЕсли;
...
Процедура ПересчитатьСуммуНДС(СтрокаПлатеж) Экспорт
	...
	ЗначениеСтавкиНДС = НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаПлатеж.СтавкаНДС);
	...
	Если ЕстьРасчетыСКонтрагентами ИЛИ ЕстьРасчетыПоКредитам Тогда
		...
		Если флВыполнитьЗаполнениеСтруктуры Тогда
			...
			Запрос.Текст = 
				...
				|	Контрагенты.ОсновнойДоговорКонтрагента.Организация.Наименование,
				|	Контрагенты.ОсновнойДоговорКонтрагента КАК ОсновнойДоговорКонтрагента,
				|	Контрагенты.ОсновнойБанковскийСчет,
				|	Контрагенты.ОсновнойБанковскийСчет.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств,
			...
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагента"						,ВыборкаДетальныеЗаписи.ОсновнойДоговорКонтрагента);
			ДанныеКонтрагент.Вставить("ОсновнойБанковскийСчет"							,ВыборкаДетальныеЗаписи.ОсновнойБанковскийСчет);
			ДанныеКонтрагент.Вставить("ОсновнойБанковскийСчетВалютаДенежныхСредств"		,ВыборкаДетальныеЗаписи.ВалютаДенежныхСредств);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаОрганизация"			,ВыборкаДетальныеЗаписи.Организация);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаВидДоговора"			,ВыборкаДетальныеЗаписи.ВидДоговора);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаВалютаВзаиморасчетов"	,ВыборкаДетальныеЗаписи.ВалютаВзаиморасчетов);
			ДанныеКонтрагент.Вставить("ОсновнойДоговорКонтрагентаТипЦен"				,ВыборкаДетальныеЗаписи.ТипЦен); 
			ДанныеКонтрагент.Вставить("ОсновноеКонтактноеЛицо"							,ВыборкаДетальныеЗаписи.ОсновноеКонтактноеЛицо);
...
Процедура ПровестиСписаниеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	Если РасчетыСКонтрагентами Тогда
		ДвиженияПоКонтрагентам.Свернуть("ДоговорКонтрагента,ВедениеВзаиморасчетов,ВестиПоДокументамРасчетовСКонтрагентом,Сделка,ДокументРасчетовСКонтрагентом,ВидДоговора, Проект","СуммаВзаиморасчетов,СуммаУпр,СуммаРегл,СуммаВзаиморасчетовОстаток,СуммаУпрОстаток");		
	КонецЕсли;
...
Процедура ПровестиПоступлениеДенежныхСредствУпр(СтруктураШапкиДокумента, СтруктураПараметров, ТаблицаПлатежейУпр, Движения, Отказ, Заголовок) Экспорт
	...
	Если Оплачено И ОтраженоВОперУчете Тогда  // Проводим по фактическим взаиморасчетам
		...
		ИначеЕсли ВидОперации = Перечисления.ВидыОперацийПКО.ПриходДенежныхСредствРозничнаяВыручка Тогда
			...
			СуммаПродаж 			= СтруктураШапкиДокумента.СуммаПродаж;
			СуммаВозврата 			= СтруктураШапкиДокумента.СуммаВозврата;
			СуммаРазменнойМонеты	= СтруктураШапкиДокумента.СуммаРазменнойМонеты;
			СуммаОплатыБанковскимиКредитами = СтруктураШапкиДокумента.СуммаОплатыБанковскимиКредитами;
			СуммаОплатыПлатежнымиКартами 	= СтруктураШапкиДокумента.СуммаОплатыПлатежнымиКартами;
