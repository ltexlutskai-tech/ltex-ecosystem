...
// Банковский счет организации
	...
	// обособленное подразеделение в налоговых документах
...
Процедура ЗаполнитьШапкуДокумента(ДокументОбъект, ТекПользователь, ВалютаРегламентированногоУчета = Неопределено, ТипОперации = "", ПараметрОбъектКопирования = Неопределено, ПараметрОснование = неопределено, СтруктураПараметровДляПолученияДоговора = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("Организация", МетаданныеДокумента) Тогда
		...
		Если НалоговыйУчет.ПолучитьСхемуНалогообложения(ОбщегоНазначения.ПолучитьРабочуюДату(),ДокументОбъект.Организация).НДС = Ложь Тогда
	...
	Если    МетаданныеДокумента.Имя = "НалоговаяНакладная"
		ИЛИ МетаданныеДокумента.Имя = "ЗаписьКнигиПриобретения" 
		ИЛИ МетаданныеДокумента.Имя = "РегистрацияВходящегоНалоговогоДокумента" Тогда
	
		Если НЕ ЗначениеЗаполнено(ДокументОбъект.ОбособленноеПодразделение) Тогда
			ДокументОбъект.ОбособленноеПодразделение = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь, "ОсновноеОбособленноеПодразделениеОрганизации");
			Если ДокументОбъект.ОбособленноеПодразделение.Владелец <> ДокументОбъект.Организация Тогда
				ДокументОбъект.ОбособленноеПодразделение = Неопределено;
			КонецЕсли;
		КонецЕсли;
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("УчитыватьНДС", МетаданныеДокумента) 
		   И (Не ДокументОбъект.УчитыватьНДС) И ОрганизацияПлательщикНДС Тогда
	...
	Если  ПараметрОснование = Неопределено И ПараметрОбъектКопирования = Неопределено
		И ОбщегоНазначения.ЕстьРеквизитДокумента("АвторасчетНДС", МетаданныеДокумента)
	...
	Если ОбщегоНазначения.ЕстьРеквизитДокумента("ДолжностьПредставителяОрганизации", МетаданныеДокумента) Тогда
	  ДокументОбъект.ДолжностьПредставителяОрганизации = УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(ТекПользователь,"ОсновнаяДолжностьПредставителяОрганизации");
  	КонецЕсли;
...
Функция НапечататьПриходРасходСРозничныхСкладовВЦенахАТТ(Док, ВалютаРегламентированногоУчета, ЭтоПриход) Экспорт
	...
	Если ЭтоПриход Тогда
		...
		ТекстВсегоПринятоОтпущено = НСтр("ru='Всего принято ';uk='Усього прийнято ' ",КодЯзыкаПечать);
	...
	Иначе
		...
		ТекстВсегоПринятоОтпущено = НСтр("ru='Всего отпущено ';uk='Усього прийнято '",КодЯзыкаПечать);
	...
	Иначе
		...
		ЗаголовокДокумента = ОпределитьЗаголовокДокумента(Док, КодЯзыкаПечать);
...
Функция ОпределитьЗаголовокДокумента(Док, КодЯзыкаПечать) Экспорт
	ИмяДокумента = Док.Метаданные().Имя;
	Если ИмяДокумента = "АвансовыйОтчет" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Авансовый отчет", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Авансовий звіт", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ВозвратТоваровОтПокупателя" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Возврат товаров от покупателя", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Повернення товарів від покупця", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ВозвратТоваровПоставщику" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Возврат товаров поставщику", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Повернення товарів постачальникові", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ВозвратТоваровПоставщикуИзНТТ" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Возврат товаров поставщику из НТТ", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Повернення товарів постачальникові із НТТ", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "КомплектацияНоменклатуры" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Комплектация номенклатуры", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Комплектація номенклатури", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "КорректировкаСерийИХарактеристикТоваров" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Корректировка серий и характеристик товаров", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Коректування серій і характеристик товарів", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ОприходованиеТоваров" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Оприходование товаров", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Оприбуткування товарів", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ОтчетКомиссионераОПродажах" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Отчет комиссионера о продажах", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Звіт комісіонера про продажі", КодЯзыкаПечать);
		КонецЕсли;	
	ИначеЕсли ИмяДокумента = "ОтчетОРозничныхПродажах" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Отчет о розничных продажах", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Звіт про роздрібні продажі", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ПеремещениеТоваров" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Перемещение товаров", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Переміщення товарів", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ПоступлениеТоваровУслуг" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Поступление товаров и услуг", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Надходження товарів і послуг", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ПоступлениеТоваровУслугВНТТ" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Поступление товаров и услуг в НТТ", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Надходження товарів і послуг у НТТ", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ПриходныйОрдерНаТовары" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Приходный ордер на товары", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Прибутковий ордер на товари", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "РасходныйОрдерНаТовары" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Расходный ордер на товары", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Видатковий ордер на товари", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "РеализацияТоваровУслуг" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Реализация товаров и услуг", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Реалізація товарів і послуг", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "СписаниеТоваров" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Списание товаров", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Списання товарів", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ТребованиеНакладная" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Требование-накладная", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Вимога-накладна", КодЯзыкаПечать);
		КонецЕсли;
	ИначеЕсли ИмяДокумента = "ЧекККМ" Тогда
		Если КодЯзыкаПечать = "ru" Тогда
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Чек ККМ", КодЯзыкаПечать);
		Иначе 
			Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, "Чек ККМ", КодЯзыкаПечать);
		КонецЕсли;
	Иначе 
		Возврат ОбщегоНазначения.СформироватьЗаголовокДокумента(Док, Док.Метаданные().Синоним, КодЯзыкаПечать);
	КонецЕсли;
КонецФункции
