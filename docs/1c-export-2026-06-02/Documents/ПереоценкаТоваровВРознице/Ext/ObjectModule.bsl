...
Функция ПечатьПереоценкиТоваровВРознице()
	...
	|	Док.Ссылка.Склад.Представление          	   КАК Склад,
	|	Док.Ссылка.Ответственный.ФизЛицо.Наименование КАК Выписал,
	|	ТоварыВРознице.НомерСтроки,
	|	Остатки.КоличествоОстаток КАК Количество,
	|	ТоварыВРознице.Номенклатура,
	|	ВЫРАЗИТЬ(ТоварыВРознице.Номенклатура.НаименованиеПолное КАК СТРОКА(1000)) КАК Товар,
