...
Функция ПечатьЧека()
	...
	Если Шапка.ВидОперации = Перечисления.ВидыОперацийЧекККМ.Возврат Тогда
		ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, "Чек (возврат)", КодЯзыкаПечать);
	Иначе
	...
	ОбластьМакета.Параметры.ИтоговаяСтрока = НСтр("ru='Всего наименований ';uk='Усього найменувань '",КодЯзыкаПечать) + ВыборкаСтрокТовары.Количество()
...
Функция ПровестиИРаспечататьЧек(Ответ, Отказ, ФормаДокумента) Экспорт
	...
	фЕстьНДС = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитики(?(ЭтоНовый(), ТекущаяДата(), Дата), Ложь, Организация, "Нал").СхемаНалогообложения.НДС;
	...
	Пока Выборка.Следующий() Цикл
		...
		Товар.СтавкаНДС    = ?(ПечататьНДСВКассовыхЧекахИзДокументовЧекККМ И фЕстьНДС, НалоговыйУчет.ПолучитьСтавкуНДС(Выборка.СтавкаНДС), 0);
...
Функция МожноРассчитыватьАвтоматическиеСкидки() Экспорт
	...
	Если мРассчитыватьАвтоматическиеСкидки = Неопределено Тогда
		Если ПолучитьЗначениеУчетнойПолитики().ИспользоватьСкидкиПоКоличествуТовара
		 ...
		 Или ЗначениеЗаполнено(КассаККМ.ПорядокОкругленияСуммыЧекаВПользуПокупателя)
