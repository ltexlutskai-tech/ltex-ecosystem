...
#КонецЕсли
	...
	ПараметрыПечати.Вставить("ТекстЗаголовка", ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Расходный ордер на товары';uk='Видатковий ордер на товари'",КодЯзыкаПечать),КодЯзыкаПечать));
	
	ПараметрыПечати.Вставить("ПредставлениеПоставщика", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "ПолноеНаименование,"));
	ПараметрыПечати.Вставить("Поставщик", Шапка.Организация);

	 ПараметрыПечати.Вставить("ПредставлениеПокупателя", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе, "ПолноеНаименование,"));
	 ПараметрыПечати.Вставить("ПредставлениеДокументаПередачи", ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка.ДокументПередачи, Локализация.ПолучитьЛокализованныйСинонимОбъекта(Шапка.ДокументПередачи.Ссылка, КодЯзыкаПечать),КодЯзыкаПечать));
	 ПараметрыПечати.Вставить("Покупатель", Шапка.Организация);
	...
	ПараметрыПечати.Вставить("ДопИнформация", МассивСтруктурСтрок);		
	...
	ПараметрыПечати.Вставить("ИтоговаяСтрока", НСтр("ru='Всего наименований ';uk='Всього найменувань '",КодЯзыкаПечать) + ВыборкаСтрокТовары.Количество()
										   + ?(ВыборкаСтрокТара.Количество() = 0, "",  НСтр("ru='; возвратная тара ';uk='; зворотна тара '",КодЯзыкаПечать) + ВыборкаСтрокТара.Количество()) + ".");
...
Функция ПечатьРасходныйОрдерНаТовары()
    ...
    Для каждого СтруктураСтроки Из ПараметрыПечати.ДопИнформация Цикл
