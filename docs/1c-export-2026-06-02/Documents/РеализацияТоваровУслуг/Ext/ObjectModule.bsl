...
Функция ПечатьДляВыгрузкиВ1СЗвит(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы = Ложь) Экспорт
	...
	Если ТолькоФормироватьСтруктуруПоказателейПечатнойФормы Тогда
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ТЧ.Цена,0) * ЕСТЬNULL(ТЧ.Количество,0) = 0
		|			ТОГДА 0
		...
		СтруктураПоказателей.ПервичныйДокумент.Вставить("РуководительПоставщика",			РуководителиОрганизации.Руководитель);
		...
		Пока ВыборкаТовары.Следующий() Цикл
			...
			СтрокаДок.СтавкаНДС = НалоговыйУчет.ПолучитьСтавкуНДС(ВыборкаТовары.СтавкаНДС);
...
Функция ПечатьДокумента(СуммыВРублях = Ложь)
	...
	|	ХарактеристикаНоменклатуры КАК Характеристика,
	|	ЕСТЬNULL(ХарактеристикаНоменклатуры.Вага, 0) КАК Вес,
	...
	|		Цена                 КАК Цена,
	|		ЦенаПродажиВес       КАК ЦенаПродажиВес,
	|		СтавкаНДС            КАК СтавкаНДС,
	...
	КонецЦикла;	
	ЗапросТовары.Свернуть("Номенклатура, Товар, Код, КодАртикул, ЕдиницаЦены, ЕдиницаМест, Цена, Характеристика, Серия", "Количество, КоличествоМест, Сумма, СуммаНДС, Скидка, АвтоматическаяСкидка");
...
Функция ПечатьАктаОбОказанииУслуг()
	...
	Если ТаблицаУслуги.Итог("Скидка") + ТаблицаУслуги.Итог("АвтоматическаяСкидка") <> 0 Тогда 
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураМакетов.Вставить("ДляВыгрузкиВ1СЗвит", "Акт/Накладная (для обмена через ""FREDO ДокМен"")");		
...
Процедура ОкруглитьСуммуДокументаПослеЗаполненияТЧ(ТабличнаяЧасть) Экспорт
	...
	ОбработкаТабличныхЧастей.ОкруглитьСуммуДокументаПослеЗаполненияТЧ(ИмяТабличнойЧасти,ДоговорКонтрагента.ПорядокОкругленияСуммыВПользуПокупателя,ТабличнаяЧасть,СуммаВключаетНДС,АвторасчетНДС,УчитыватьНДС);
КонецПроцедуры 
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ОтражатьВУправленческомУчете Тогда
		...
		Если НЕ (ЗначениеЗаполнено(МаршрутныйЛист) И НЕ МаршрутныйЛист.ПометкаУдаления И МаршрутныйЛист.СтатусДокумента <> Перечисления.СтатусыМаршрутногоЛиста.Завершен) Тогда  // По незавершених мар шрутниках реалызацыъ не списують товар зы складу
		Если ВидПередачи = Перечисления.ВидыПередачиТоваров.СоСклада Тогда
			...
			Если Не РезультатЗапросаКомплектыОпт.Пустой() ИЛИ НЕ РезультатЗапросаТараОпт.Пустой() Тогда
				...
				Если НЕ Отказ Тогда
					...
					СтруктТаблицДокумента = Новый Структура;
					ТЗТовары = РезультатЗапросаКомплектыОпт.Выгрузить();
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре) Экспорт
	...
	СтруктураПолей.Вставить("Вес"                      			 , "Количество * ХарактеристикаНоменклатуры.Вага");
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Дата >= Дата('20260117') Тогда 
		//Общий.ОкруглитьСуммуДоГривни(Товары);
	КонецЕсли;
	...
	Для каждого СтрокаТабличнойЧасти из Товары цикл
		СтрокаТабличнойЧасти.СпособСписанияОстаткаТоваров = Перечисления.СпособыСписанияОстаткаТоваров.СоСклада;  // пока отказались от резервирования, поскольку легко могут продать товар из чужого заказа.
	КонецЦикла;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Не Отказ Тогда
		//ДвиженияВеса.ДвижениеРасходВеса(ВидДвиженияНакопления.Приход,Склад, Товары);	
		
		Артикулы  = Товары.Выгрузить(,"Номенклатура");
		Артикулы.Свернуть("Номенклатура");
		Для Каждого Элемент Из Артикулы Цикл 
			Запись = РегистрыСведений.АсортиментПокупателя.СоздатьМенеджерЗаписи();
...
// ПРОЦЕДУРЫ И ФУНКЦИИ ФОРМИРОВАНИЯ ЭКСПРЕСС-НАКЛАДНОЙ +++
Функция СформироватьЭкспрессНакладную() Экспорт 
	
	Успех 						= Ложь;
	
	Если Не ПроверитьТипДокумента() Тогда
		Возврат Успех;
	КонецЕсли;
	
	Если НомерЭкспрессНакладной = "" Тогда
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Для документа %1 не вказаний номер експрес-накладної. Вкажіть його в документі.", Ссылка), Статуссообщения.Важное);
		Возврат Успех;
	КонецЕсли;
	...
	ТекущаяНастройка 			= Организация.НастройкаАвторизацииCheckBox;
	Если НЕ ТекущаяНастройка 	= Справочники.НастройкиАвторизацииCheckBox.ПустаяСсылка() Тогда 
		НастройкиПодключения 	= ПолучитьНастройкиПодключения(НомерЭкспрессНакладной, ТекущаяНастройка);
		СтруктураЗапроса 		= СформироватьСтруктуруЗапроса(ДанныеДокумента, НастройкиПодключения);
		СтрокаJSON 				= СериализоватьВJSON(СтруктураЗапроса);
	Иначе 
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Для організації %1 не вказане налаштування CheckBox, документ не сформований.", Организация), СтатусСообщения.Важное);
		Возврат Успех;
	КонецЕсли;	
	
	Успех 						= ВыполнитьЗапросКAPI(СтрокаJSON, НастройкиПодключения, Ссылка);
	
	Возврат Успех;
	
КонецФункции

Функция ПроверитьТипДокумента()
	
	Если ТипЗнч(Ссылка) <> Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		Сообщить("Невірний тип документа. Очікується видаткова накладна.");
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьДанныеДокумента()
	...
	Запрос.Текст 	= 
	"ВЫБРАТЬ
	|	РеализацияТоваровУслугТовары.Ссылка.Ответственный КАК Сотрудник,
	|	""Карта"" КАК ВидОплаты,
	|	&КассаККМ КАК КассаККМ,
	|	ВЫБОР
	|		КОГДА РеализацияТоваровУслугТовары.Номенклатура.Название = ""Одяг вживаний""
	|			ТОГДА ""1""
	|		ИНАЧЕ ""2""
	|	КОНЕЦ КАК Код,
	|	РеализацияТоваровУслугТовары.ХарактеристикаНоменклатуры КАК Характеристика,
	|	РеализацияТоваровУслугТовары.Номенклатура.Название КАК Наименование,
	|	СУММА(ВЫБОР
	...
	|		КОНЕЦ) КАК ЦенаВКопейках,
	|	СУММА(РеализацияТоваровУслугТовары.Сумма) КАК СуммаРасчитанная,
	|	СУММА(РеализацияТоваровУслугТовары.Сумма) КАК Сумма,
	|	СУММА(РеализацияТоваровУслугТовары.Количество) КАК КоличествоВТысячах,
	|	РеализацияТоваровУслугТовары.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(РеализацияТоваровУслугТовары.Ссылка.СуммаДокумента) КАК СуммаДокумента,
	|	РеализацияТоваровУслугТовары.ХарактеристикаНоменклатуры.Вага КАК Вес,
	|	СУММА(0) КАК Скидка
	|ИЗ
	...
	|	РеализацияТоваровУслугТовары.Номенклатура.Название,
	|	ВЫБОР
	|		КОГДА РеализацияТоваровУслугТовары.Номенклатура.Название = ""Одяг вживаний""
	|			ТОГДА ""1""
	|		ИНАЧЕ ""2""
	|	КОНЕЦ,
	|	РеализацияТоваровУслугТовары.ХарактеристикаНоменклатуры.Вага";	
	
	Запрос.УстановитьПараметр("Документ", Ссылка);
	Запрос.УстановитьПараметр("КассаККМ", Справочники.КассыККМ.НайтиПоКоду("000000001"));
	
	Результат = Запрос.Выполнить();
	
	Если Результат.Пустой() Тогда
		Сообщить("Дані документа не знайдені.");
		Возврат Неопределено;
	КонецЕсли;
	...
	Для Каждого СтрРезультат Из ТЗРезультат Цикл
    	СтрРезультат.КоличествоВТысячах = ?(СтрРезультат.Вес = NULL, СтрРезультат.КоличествоВТысячах, СтрРезультат.Вес);
	КонецЦикла;
	
	ИтогоСуммаРасчитанная = ТЗРезультат.Итог("СуммаРасчитанная");

	Если ИтогоСуммаРасчитанная <> 0 Тогда
	    РаспределеноВсего = 0;
	    Для Индекс = 0 По ТЗРезультат.Количество() - 1 Цикл
	        СтрРезультат = ТЗРезультат[Индекс];
	        Если Индекс < ТЗРезультат.Количество() - 1 Тогда
	            СтрРезультат.Сумма = Окр(СумаОплатиНаложкою * СтрРезультат.СуммаРасчитанная / ИтогоСуммаРасчитанная, 2);
	            РаспределеноВсего = РаспределеноВсего + СтрРезультат.Сумма;
	        Иначе
	            СтрРезультат.Сумма = СумаОплатиНаложкою - РаспределеноВсего;
			КонецЕсли;
			СтрРезультат.Сумма = СтрРезультат.Сумма;
	        Если СтрРезультат.КоличествоВТысячах <> 0 Тогда
	            СтрРезультат.ЦенаВКопейках = Окр(СтрРезультат.Сумма / СтрРезультат.КоличествоВТысячах, 0);
	        КонецЕсли;
	    КонецЦикла;
	КонецЕсли;	
...
Функция ПолучитьНастройкиПодключения(НомерЭкспрессНакладной, ТекущаяНастройка)
	
	ТокенАвторизации	= ПолучитьТокенАвторизацииПоПинКоду(ТекущаяНастройка.ПинКод, ТекущаяНастройка.НазваниеКлиента, ТекущаяНастройка.ВерсияКлиента, ТекущаяНастройка.КлючЛицензии);

	Настройки = Новый Структура;
	Настройки.Вставить("URL", "https://api.checkbox.ua/api/v1/np/ettn");
	Настройки.Вставить("ТокенАвторизации", "Bearer " + ТокенАвторизации);
	Настройки.Вставить("КлючЛицензии", ТекущаяНастройка.КлючЛицензии);   
	Настройки.Вставить("НазваниеКлиента", ТекущаяНастройка.НазваниеКлиента);
	Настройки.Вставить("ВерсияКлиента", ТекущаяНастройка.ВерсияКлиента);
	Настройки.Вставить("НомерЭкспрессНакладной", НомерЭкспрессНакладной);
	
	Возврат Настройки;
	
КонецФункции

Функция ПолучитьТокенАвторизации(Логин, Пароль, НазваниеКлиента, ВерсияКлиента) Экспорт
	...
	HTTPСоединение = Новый HTTPСоединение("api.checkbox.ua", 443,,,,, Новый ЗащищенноеСоединениеOpenSSL());
	HTTPЗапрос = Новый HTTPЗапрос("/api/v1/cashier/signin");
	
	HTTPЗапрос.Заголовки.Вставить("accept", "application/json");
	HTTPЗапрос.Заголовки.Вставить("X-Client-Name", НазваниеКлиента);
	HTTPЗапрос.Заголовки.Вставить("X-Client-Version", ВерсияКлиента);
	...
	Попытка
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			...
			Если ДанныеОтвета.Свойство("access_token") Тогда
				//Сообщить("Токен отримано: " + ДанныеОтвета.access_token);
				Возврат ДанныеОтвета.access_token;
			Иначе
				Сообщить("У відповіді API відсутній токен авторизації.");
				Возврат "";
			КонецЕсли;
		Иначе
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Помилка авторизації. Код: %1. Текст: %2", HTTPОтвет.КодСостояния, ТекстОтвета), СтатусСообщения.Важное);
			Возврат "";
		КонецЕсли;
		
	Исключение
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Помилка вконання запиту: %1", ОписаниеОшибки()), СтатусСообщения.Важное);
		Возврат "";
	КонецПопытки;
	
КонецФункции

Функция ПолучитьТокенАвторизацииПоПинКоду(ПинКод, НазваниеКлиента, ВерсияКлиента, КлючЛицензии) Экспорт
	
	СтруктураЗапроса = Новый Структура;
	СтруктураЗапроса.Вставить("pin_code", ПинКод);
	
	СтрокаJSON = СериализоватьВJSON(СтруктураЗапроса);
	
	HTTPСоединение = Новый HTTPСоединение("api.checkbox.ua", 443,,,,, Новый ЗащищенноеСоединениеOpenSSL());
	HTTPЗапрос = Новый HTTPЗапрос("/api/v1/cashier/signinPinCode");
	
	HTTPЗапрос.Заголовки.Вставить("accept", "application/json");
	HTTPЗапрос.Заголовки.Вставить("X-Client-Name", НазваниеКлиента);
	HTTPЗапрос.Заголовки.Вставить("X-Client-Version", ВерсияКлиента);
	HTTPЗапрос.Заголовки.Вставить("X-License-Key", КлючЛицензии);
	HTTPЗапрос.Заголовки.Вставить("Content-Type", "application/json");
...
Функция Авторизоваться(Логин, Пароль) Экспорт
	
	Соединение = Новый HTTPСоединение("api.checkbox.in.ua", Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Новый ЗащищенноеСоединениеOpenSSL);
	...
	Иначе
		...
		Если Не (Результат = Неопределено) Тогда
			...
			Возврат Результат.message;
		КонецЕсли;
	КонецЕсли;
	
КонецФункции

Функция СформироватьСтруктуруЗапроса(ТЗДанныеДокумента, Настройки)
	
	СтруктураЗапроса 	= Новый Структура;
	СтруктураТелаЧека = Новый Структура;

	//Товары
	МассивТоваров 			= Новый Массив;
	СуммаТоваровВКопейках   = 0;
	Для Каждого ДанныеДокумента Из ТЗДанныеДокумента Цикл
		СтруктураЗапроса.Вставить("employee", "");
		СтруктураЗапроса.Вставить("cashRegister", "");
		
		СтруктураТовара = Новый Структура;
		ДанныеТовара = Новый Структура;
		ДанныеТовара.Вставить("code", ДанныеДокумента.Код);
		ДанныеТовара.Вставить("name", ДанныеДокумента.Наименование);
		ДанныеТовара.Вставить("price", ДанныеДокумента.ЦенаВКопейках * 100);
		
		МассивНалогов = Новый Массив;
		МассивНалогов.Добавить(ПолучитьКодНДС(ДанныеДокумента.СтавкаНДС));
		ДанныеТовара.Вставить("tax", МассивНалогов);
		
		СтруктураТовара.Вставить("good", ДанныеТовара);
		СтруктураТовара.Вставить("quantity", ДанныеДокумента.КоличествоВТысячах * 1000);
		СтруктураТовара.Вставить("is_return", Ложь);
		
		МассивТоваров.Добавить(СтруктураТовара);
		СуммаТоваровВКопейках = СуммаТоваровВКопейках + ДанныеДокумента.ЦенаВКопейках * ДанныеДокумента.КоличествоВТысячах;
	КонецЦикла;	
    СуммаТоваровВКопейках = СуммаТоваровВКопейках * 100;
	
	СтруктураТелаЧека.Вставить("goods", МассивТоваров);
		
	//Оплата
	СуммаОплатыВКопейках = (СумаОплатиНаложкою * 100);
	МассивПлатежей = Новый Массив;
	СтруктураПлатежа = Новый Структура;
	СтруктураПлатежа.Вставить("type", "ETTN");
	СтруктураПлатежа.Вставить("label", "Платіж через інтегратора NovaPay");
	СтруктураПлатежа.Вставить("value", СуммаОплатыВКопейках);
	СтруктураПлатежа.Вставить("ettn", Настройки.НомерЭкспрессНакладной);
	
	МассивПлатежей.Добавить(СтруктураПлатежа);
	СтруктураТелаЧека.Вставить("payments", МассивПлатежей);
	
	Разница = СуммаТоваровВКопейках - СуммаОплатыВКопейках;
	Если Цел(Разница) <> 0 Тогда
		МассивСкидок = Новый Массив;
		СтруктураСкидок = Новый Структура;
		Если Разница > 0 Тогда
			СтруктураСкидок.Вставить("type", "DISCOUNT");
			СтруктураСкидок.Вставить("name", "Знижка");
		Иначе
			СтруктураСкидок.Вставить("type", "EXTRA_CHARGE");
			СтруктураСкидок.Вставить("name", "Націнка");
			Разница = -Разница;
		КонецЕсли;
		
		СтруктураСкидок.Вставить("mode", "VALUE");
		СтруктураСкидок.Вставить("value", Цел(Разница));
		
		МассивСкидок.Добавить(СтруктураСкидок);
		СтруктураТелаЧека.Вставить("discounts", МассивСкидок);
	КонецЕсли;
	
	СтруктураТелаЧека.Вставить("footer", "Дякуємо за покупку!");
	СтруктураЗапроса.Вставить("receipt_body", СтруктураТелаЧека);
	
	Возврат СтруктураЗапроса;
	
КонецФункции

Функция СериализоватьВJSON(СтруктураЗапроса)
...
Функция ВыполнитьЗапросКAPI(СтрокаJSON, Настройки, СсылкаНаДокумент)
	
	HTTPСоединение = Новый HTTPСоединение("api.checkbox.ua", 443,,,,, Новый ЗащищенноеСоединениеOpenSSL());
	HTTPЗапрос = Новый HTTPЗапрос("/api/v1/np/ettn");
	
	HTTPЗапрос.Заголовки.Вставить("accept", "application/json");
	HTTPЗапрос.Заголовки.Вставить("Authorization", Настройки.ТокенАвторизации);
	HTTPЗапрос.Заголовки.Вставить("X-Client-Name", Настройки.НазваниеКлиента); 
	HTTPЗапрос.Заголовки.Вставить("X-Client-Version", Настройки.ВерсияКлиента);
	HTTPЗапрос.Заголовки.Вставить("X-License-Key", Настройки.КлючЛицензии);
	HTTPЗапрос.Заголовки.Вставить("Content-Type", "application/json");
	...
	Попытка
		...
		Если HTTPОтвет.КодСостояния = 200 Или HTTPОтвет.КодСостояния = 201 Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Експрес-накладна для документа %1 успішно створена: %2", СсылкаНаДокумент), СтатусСообщения.Важное);
			Возврат Истина;
		Иначе
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Помилка створення експрес-накладної для %1. Код: %2. Текст: %3", 
				СсылкаНаДокумент, HTTPОтвет.КодСостояния, ТекстОтвета), СтатусСообщения.Важное);
			Возврат Ложь;
		КонецЕсли;
		
	Исключение
		Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Помилка виконання запиту для %1: %2", СсылкаНаДокумент, ОписаниеОшибки()), СтатусСообщения.Важное);
		Возврат Ложь;
	КонецПопытки;
	
КонецФункции

Функция ПолучитьКодНДС(СтавкаНДС)
