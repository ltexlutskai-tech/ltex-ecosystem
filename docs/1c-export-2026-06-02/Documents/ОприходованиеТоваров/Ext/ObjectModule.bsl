...
#КонецЕсли
	...
	ПараметрыПечати.Вставить("ТекстЗаголовка", ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Оприходование товаров';uk='Оприбуткування товарів'",КодЯзыкаПечать),КодЯзыкаПечать));
	...
	ПараметрыПечати.Вставить("РеквизитыПоставщика", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "ЮридическийАдрес,Телефоны,/,КодПоЕДРПОУ,ИНН,НомерСвидетельства,",,КодЯзыкаПечать));
	ПараметрыПечати.Вставить("Склад", Шапка.Склад);
	ПараметрыПечати.Вставить("Валюта", Шапка.Валюта);
	ПараметрыПечати.Вставить("Основание", Шапка.Основание);
	ПараметрыПечати.Вставить("ПредставлениеСклада", Шапка.Склад);
	ПараметрыПечати.Вставить("ПредставлениеВалюты", СокрЛП(ВалютаПечати));
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	КонецЕсли;
	Если Не Отказ Тогда
		ДвиженияВеса.ДвижениеПриходВеса(ВидДвиженияНакопления.Приход,Склад, Товары);	
	КонецЕсли;
