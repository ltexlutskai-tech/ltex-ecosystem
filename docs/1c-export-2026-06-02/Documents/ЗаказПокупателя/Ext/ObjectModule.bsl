...
Функция ПолучитьПараметрыПечатиСчетаЗаказа(Тип) Экспорт	
	...
	ПараметрыПечати.Вставить("ПредставлениеРеквизитовПоставщика", "П/р " + НомерСчета + ", Банк " + Банк + ", МФО " + МФО + Символы.ПС
													 + ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике, "ЮридическийАдрес,Телефоны,/,КодПоЕДРПОУ,ИНН,НомерСвидетельства,/,ИнформацияОСтатусеПлательщикаНалогов,",,"uk"));
	...
	Если ЗапросТовары.Итог("Цена")*ЗапросТовары.Итог("Количество")  <> 0 Тогда 
		ИтоговыйПроцентСкидки = (1 - ЗапросТовары.Итог("Сумма") / (ЗапросТовары.Итог("Цена")*ЗапросТовары.Итог("Количество"))) * 100;
		ЕстьСкидки = Ценообразование.ПолучитьСуммуСкидки(ЗапросТовары.Итог("Сумма"), ИтоговыйПроцентСкидки) >= 0.01;
	Иначе
		ЕстьСкидки = Ложь;
	КонецЕсли;
...
Функция СоздатьТаблицуРезервированияПодЗаказ(ТаблицаПоКомплектам)
	...
	// резервируются только реальные остатки товаров
	...
	ЗапросОстатков.УстановитьПараметр("Период", ГраницаПериода);
	ЗапросОстатков.УстановитьПараметр("Склад", СкладГруппа);
	ЗапросОстатков.УстановитьПараметр("МассивНоменклатуры", Товары.ВыгрузитьКолонку("Номенклатура"));
	ТЗОстаткиТоваров = ЗапросОстатков.Выполнить().Выгрузить();
	ТЗОстаткиТоваров.Индексы.Добавить("Склад, Номенклатура, ХарактеристикаНоменклатуры, СерияНоменклатуры, Качество");
	КачествоНовый 	 = Справочники.Качество.Новый;
	НоваяТаблицаПоТоварамРезервирование = ТаблицаПоТоварамРезервирование.СкопироватьКолонки();
	Для Каждого СтрРезерва Из ТаблицаПоТоварамРезервирование Цикл 
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Склад", СтрРезерва.Склад);
		СтруктураПоиска.Вставить("Номенклатура", СтрРезерва.Номенклатура);
		МассивОстатков = ТЗОстаткиТоваров.НайтиСтроки(СтруктураПоиска);
		КоличествоКРезервированию = СтрРезерва.Количество;
		Если МассивОстатков.Количество() > 0 Тогда 
			Для Каждого СтрМассива Из МассивОстатков Цикл 
				Если СтрРезерва.Склад = СтрМассива.Склад 
					И СтрРезерва.Номенклатура = СтрМассива.Номенклатура 
					И СтрРезерва.ХарактеристикаНоменклатуры = СтрМассива.ХарактеристикаНоменклатуры Тогда 
						СтрРезерва.Количество = Мин(СтрРезерва.Количество, СтрМассива.КоличествоОстаток);
						КоличествоКРезервированию = КоличествоКРезервированию - СтрРезерва.Количество;
						СтрУдалить = СтрМассива;
						Прервать;
				КонецЕсли;		
			КонецЦикла;
			ИндексУдалить = МассивОстатков.Найти(СтрУдалить);
			Если ИндексУдалить <> Неопределено Тогда 
				МассивОстатков.Удалить(ИндексУдалить);
			КонецЕсли;	
			Если КоличествоКРезервированию > 0 Тогда 
				Для Каждого СтрМассива Из МассивОстатков Цикл 
					НоваяСтрока = НоваяТаблицаПоТоварамРезервирование.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрРезерва);
					НоваяСтрока.Количество = Мин(КоличествоКРезервированию, СтрМассива.КоличествоОстаток);
					КоличествоКРезервированию = КоличествоКРезервированию - НоваяСтрока.Количество;
					Если КоличествоКРезервированию <= 0 Тогда 
						Прервать;
					КонецЕсли;	
				КонецЦикла;				
			КонецЕсли;
		Иначе 
			СтрРезерва.Количество = 0;
			# Если Клиент Тогда 
				Сообщить("По номенклатуре " + СтрРезерва.Номенклатура + " неправильные остатки. Резервирование невозможно.");
			# КонецЕсли	
		КонецЕсли;	
	КонецЦикла;	
	ТаблицаПоТоварамРезервирование.Очистить();
	Для Каждого СтрНовая Из НоваяТаблицаПоТоварамРезервирование Цикл 
		Если СтрНовая.Количество > 0 Тогда  
			СтрРезерва = ТаблицаПоТоварамРезервирование.Добавить();
			ЗаполнитьЗначенияСвойств(СтрРезерва, СтрНовая);
		КонецЕсли;
	КонецЦикла;			
	//-	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого Строка из Товары цикл
		...
		Иначе 	
			Строка.Размещение = СкладГруппа;
		КонецЕсли;
		//Строка.Размещение = Неопределено; // пока отказались от резервирования, поскольку легко могут продать товар из чужого заказа.
	КонецЦикла;
	
	Если ЭтоНовый() Тогда 
		СтатусЗамовлення = Справочники.СтатусиЗамовлень.Новий;
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// резервируются только реальные остатки, 
	// поэтому блокируем их на чтение до проведения документа
	...
	ЭлементБлокировки.УстановитьЗначение("Склад", СкладГруппа);
