...
Функция ПечатьАвансовогоОтчета()
	...
	ОбластьМакета.Параметры.Израсходовано = Шапка.СуммаДокумента;
	ОбластьМакета.Параметры.СуммаОтчетПодтверждаю = ОбщегоНазначения.СформироватьСуммуПрописью(Шапка.СуммаДокумента, ВалютаДокумента, КодЯзыкаПечать);
	...
	ЗапросТовары.Текст ="ВЫБРАТЬ
	                    ...
	                    |	ДанныеАвансовогоОтчета.Дата,
	                    |	ДанныеАвансовогоОтчета.Поставщик,
	                    |	ДанныеАвансовогоОтчета.ВидДокВходящий,
	                    |	ДанныеАвансовогоОтчета.Содержание,
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	ДополнитьСтруктуруОбязательныхПолейТабличнойЧастиТоварыРегл(СтруктураОбязательныхПолей);
...
Процедура ПроверитьЗаполнениеТабличнойЧастиПрочее(ТаблицаПоПрочее, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	ДополнитьСтруктуруОбязательныхПолейТабличнойЧастиПрочееРегл(СтруктураОбязательныхПолей);
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 	                
	...
	ДвиженияПоЕдиномуНалогу(РежимПроведения, СтруктураШапкиДокумента,
	                ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоВыданныеАвансы, ТаблицаПоОплатамПоставщикам, ТаблицаПоПрочее,
	                Отказ, Заголовок);
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоПрочее) Экспорт
	...
	СтруктураПолей.Вставить("СтатьяДекларацииПоЕдиномуНалогу"
	...
	ТаблицаПоТоварам = ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента, мПогрешностиОкругления);
	ТаблицаПоПрочее  = ПодготовитьТаблицуПрочие(РезультатЗапросаПоПрочее, СтруктураШапкиДокумента, мПогрешностиОкругления);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = НалоговыйУчет.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Товары")
		...
		+ НалоговыйУчет.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Прочее");
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ТаблицаПоОплате  = ПодготовитьТаблицуПоОплате(РезультатЗапросаПоОплатеПоставщикам, СтруктураШапкиДокумента, мПогрешностиОкругления);
...
Процедура ДвиженияРегистровПодсистемыНДС(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре,  ТаблицаПоОплатамПоставщикам, ТаблицаПоПрочее, Отказ, Заголовок);
	...
	ТаблицаКопия.Свернуть("СтавкаНДС, ДоговорКонтрагента, Внешнеэкономический","СуммаСНДСРегл,СуммаНДСРегл");
	...
	ТаблицаПриобретений.Свернуть("СтавкаНДС,ДоговорКонтрагента,Внешнеэкономический","СуммаСНДСРегл,СуммаНДСРегл");
	...
	СтрокиНерезидента = ТаблицаПриобретений.НайтиСтроки(Новый Структура("Внешнеэкономический", Истина));
	Для каждого СтрокаНерезидента Из СтрокиНерезидента Цикл
		ТаблицаПриобретений.Удалить(СтрокаНерезидента);
	КонецЦикла; 
	...
	ТаблицаКопия.Свернуть("ДоговорКонтрагента,Внешнеэкономический","СуммаСНДСРегл");
	ТаблицаКопия.Колонки.СуммаСНДСРегл.Имя = "СуммаВзаиморасчетовРегл";	
	
	// приобретение у нерезидента не отражается в подсистеме учета НДС
	СтрокиНерезидента = ТаблицаКопия.НайтиСтроки(Новый Структура("Внешнеэкономический", Истина));
	Для каждого СтрокаНерезидента Из СтрокиНерезидента Цикл
		ТаблицаКопия.Удалить(СтрокаНерезидента);
	КонецЦикла; 
	...
	Если НЕ Отказ И ТаблицаДвижений.Количество() > 0 Тогда
		...
		Движения.НДСПриобретений.Записать();
				...
				// За все это подотчетник уже заплатил, поэтому зарегистрируем оплату в регистре
		ТаблицаДвижений.ЗаполнитьЗначения(Перечисления.СобытияНДСПриобретений.ОплатаПоставщику	, "СобытиеНДС");
		ТаблицаДвижений.ЗаполнитьЗначения(Перечисления.СтавкиНДС.ПустаяСсылка()					, "СтавкаНДС");
		ТаблицаДвижений.ЗаполнитьЗначения(0				, "НДСВходящий");
...
Процедура ПодготовитьТаблицуПоОплатеРегл(ТаблицаПоОплате, СтруктураШапкиДокумента, ПогрешностиОкругления)
	...
	ТаблицаПоОплате.Колонки.Добавить("СуммаСНДСРегл", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));
	ТаблицаПоОплате.Колонки.Добавить("СуммаНДСРегл", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));

	ВалютаРег      = мВалютаРегламентированногоУчета;
	Данные         = МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаРег, Дата);
	
	// Надо расчитать сумму с НДС.
	Для каждого СтрокаТаблицы Из ТаблицаПоОплате Цикл

		Если СтрокаТаблицы.ВалютаДоговора = ВалютаРег Тогда
			СтрокаТаблицы.СуммаСНДСРегл    = СтрокаТаблицы.СуммаВзаиморасчетов;
			СтрокаТаблицы.СуммаНДСРегл = СтрокаТаблицы.СуммаНДС;
		Иначе
			СтрокаТаблицы.СуммаСНДСРегл = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СтрокаТаблицы.Сумма, СтруктураШапкиДокумента.ВалютаДокумента,
			                                 ВалютаРег,
			                                 СтруктураШапкиДокумента.КурсДокумента,
			                                 Данные.Курс,
			                                 СтруктураШапкиДокумента.КратностьДокумента,
...
Процедура ПодготовитьТаблицуПрочиеРегл(ТаблицаПрочие, СтруктураШапкиДокумента, ПогрешностиОкругления)
	...
	ПодготовитьТаблицуТоваровРегл(ТаблицаПрочие, СтруктураШапкиДокумента, ПогрешностиОкругления);	
	
КонецПроцедуры


Процедура ДополнитьСтруктуруОбязательныхПолейТабличнойЧастиТоварыРегл(СтруктураОбязательныхПолей)
...
Процедура ДополнитьСтруктуруОбязательныхПолейТабличнойЧастиПрочееРегл(СтруктураОбязательныхПолей)
...
Процедура ДвиженияПоЕдиномуНалогу(РежимПроведения, СтруктураШапкиДокумента, 
	ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоВыданныеАвансы, ТаблицаПоОплатамПоставщикам, ТаблицаПоПрочее,
	Отказ, Заголовок)
	// По налоговому учету можем проводить при выключенном флажке бухгалтерский учет
	Если ОтражатьВНалоговомУчете Тогда
		Если мТекущаяСхемаНалогообложения.ЕдиныйНалог Тогда
			...
			ТаблицаПлатежей.Колонки.Добавить("Сумма",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
			ТаблицаПлатежей.Колонки.Добавить("СуммаНДС",Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
			...
			ТаблицаКопия.Колонки.СуммаСНДСРегл.Имя = "Сумма";
			ТаблицаКопия.Колонки.СуммаНДСРегл.Имя = "СуммаНДС";
			ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаКопия, ТаблицаПлатежей);
		...
		//	Если НЕ (РеестрПлатежейПоЕдиномуНалогу = Неопределено) Тогда
		//		РеестрПлатежейПоЕдиномуНалогу.Очистить();
		//	КонецЕсли;
		//	
		//	Если ТаблицаПоТоварам.Количество()>0 Тогда
		//		ДописатьРеестрПлатежей(ТаблицаПоТоварам);
		//	КонецЕсли;
		//
		//	Если ТаблицаПоОплатамПоставщикам.Количество()>0 Тогда
		//		ДописатьРеестрПлатежей(ТаблицаПоОплатамПоставщикам);
		//	КонецЕсли;
		//
		//	Если ТаблицаПоПрочее.Количество()>0 Тогда
		//		ДописатьРеестрПлатежей(ТаблицаПоПрочее);
		//	КонецЕсли;
		//	
		//	Если НЕ (РеестрПлатежейПоЕдиномуНалогу = Неопределено) Тогда
		//		РеестрПлатежейПоЕдиномуНалогу.Свернуть("СтатьяДекларацииПоЕдиномуНалогу", "Сумма");
		//		ТаблицаПлатежей = РеестрПлатежейПоЕдиномуНалогу.Скопировать();
		//	КонецЕсли;
			...
			Для Каждого СтрокаПлатеж Из ТаблицаПлатежей Цикл
				...
				Если мТекущаяСхемаНалогообложения.НДС И Дата >= '20120101' Тогда
					СтрокаПлатеж.Сумма    	  = (СтрокаПлатеж.Сумма - СтрокаПлатеж.СуммаНДС);
				КонецЕсли;
