...
Процедура ДвиженияПоРегиструСписанныеТовары(РежимПроведения, ТаблицаЗакрываемыеЗаказы, Отказ, Заголовок)
	...
	ТаблицаСРаздельнымУчетом.Свернуть("ОбособленныйУчетТоваровПоЗаказамПокупателей,Номенклатура,ХарактеристикаНоменклатуры,ЗаказПокупателя");
...
Процедура ПриЗаписи(Отказ)
	...
	ДЛя Каждого Элемент Из Заказы Цикл 
		...
		мЗаказ = Элемент.ЗаказПокупателя.ПолучитьОбъект();
		мЗаказ.СтатусЗамовлення = Справочники.СтатусиЗамовлень.Неактуальний;
		мЗаказ.ОбменДанными.Загрузка = Истина;
		мЗаказ.Записать();
