...
Функция ПечатьПланаЗакупок()
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='План покупок';uk='План закупівель'",КодЯзыкаПечать),КодЯзыкаПечать);
	...
	ОбластьМакета.Параметры.ПериодичностьПредставление = Локализация.ПолучитьЛокализованноеПредставление(Шапка.Периодичность, КодЯзыкаПечать);
...
Функция ПодготовитьТаблицуСоставПлана(РезультатЗапросаПоСоставПлана, ВыборкаПоШапкеДокумента)
	...
	Для каждого СтрокаТаблицы Из ТаблицаСоставПлана Цикл
		...
		// Суммы пересчитаем е валюту упр. учета
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = НалоговыйУчет.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "СоставПлана");
