...
//
Функция ПечатьПлатежногоТребованияПоручения(Вариант2009) Экспорт
	...
	Если Вариант2009 Тогда
		...
		Если НачалоДня(Дата) > глЗначениеПеременной("ДатаОкончанияПереходаНаСтандартIBAN") Тогда
			Макет = ПолучитьОбщийМакет("ПлатежноеТребованиеПоручение_2020");
			МакетАдаптированДляIBAN = Истина;
		Иначе
			Макет = ПолучитьОбщийМакет("ПлатежноеТребованиеПоручение2009");
		КонецЕсли;
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Платежное требование-поручение" ИЛИ ИмяМакета = "ПлатежноеТребованиеПоручение" Тогда
		 
		// Управленческая печатная форма документа
		ТабДокумент = ПечатьПлатежногоТребованияПоручения(Истина);
		
	ИначеЕсли ИмяМакета = "Платежное требование-поручение, старая версия" ИЛИ ИмяМакета = "ПлатежноеТребованиеПоручениеДо2009" Тогда
