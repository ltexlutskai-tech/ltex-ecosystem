...
Функция ПечатьАккредитива() Экспорт
	...
	СведенияОПлательщике = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Организация, Дата,,СчетОрганизации, "uk");
	
	ОбластьМакета.Параметры.НазваниеОрганизации                 = СведенияОПлательщике.ПолноеНаименование;
	ОбластьМакета.Параметры.ЕДРПОУОрганизации                   = УправлениеКонтактнойИнформацией.ПолучитьКодОрганизации(СведенияОПлательщике);
	ОбластьМакета.Параметры.НомерРасчетногоСчетаОрганизации     = СведенияОПлательщике.НомерСчетаСПереносом;
	ОбластьМакета.Параметры.БанкОрганизации                     = СведенияОПлательщике.Банк;
	ОбластьМакета.Параметры.МФОБанкаОрганизации                 = СведенияОПлательщике.МФО_Платеж;
	
	СведенияОПолучателе = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Контрагент, Дата,,СчетКонтрагента, "uk");
	
	ОбластьМакета.Параметры.НазваниеЮрФизЛицаКонтрагента                = СведенияОПолучателе.ПолноеНаименование;
	ОбластьМакета.Параметры.ЕДРПОУЮрФизЛицаКонтрагента                  = УправлениеКонтактнойИнформацией.ПолучитьКодОрганизации(СведенияОПолучателе);
	ОбластьМакета.Параметры.НомерРасчетногоСчетаЮрФизЛицаКонтрагента    = СведенияОПолучателе.НомерСчетаСПереносом;
	ОбластьМакета.Параметры.БанкЮрФизЛицаКонтрагента                    = СведенияОПолучателе.Банк;
	ОбластьМакета.Параметры.МФОБанкаЮрФизЛицаКонтрагента                = СведенияОПолучателе.МФО_Платеж;
	...
	КонецЕсли;
	ОбластьМакета.Параметры.СуммаСНДССтрокой  = ОбщегоНазначения.СформироватьСуммуПрописью(СуммаДокумента, мВалютаРегламентированногоУчета,"uk");
	ТабДокумент.Вывести(ОбластьМакета);
	...
	ОбластьМакета.Параметры.НомерДоговора         = "";
	ОбластьМакета.Параметры.ЧислоДатыДоговора     = """      """;
	ОбластьМакета.Параметры.МесяцДатыДоговора     = "                                     р.";
	ТабДокумент.Вывести(ОбластьМакета);
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, Отказ, Заголовок, СтруктураШапкиДокумента)
	...
	ДвиженияПоКонтрагентам.Свернуть("ДоговорКонтрагента,ВестиПоДокументамРасчетовСКонтрагентом,Сделка,ДокументРасчетовСКонтрагентом,ВидДоговора, Проект","СуммаВзаиморасчетов,СуммаУпр,СуммаРегл,СуммаВзаиморасчетовОстаток,СуммаУпрОстаток");
