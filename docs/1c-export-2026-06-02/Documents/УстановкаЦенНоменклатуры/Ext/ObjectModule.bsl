...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаПоТоварам, Отказ, Заголовок)
	...
	НаборДвижений.мКонтрольАкций   = ЗначениеЗаполнено(ДокументОснование);
	НаборДвижений.мДопСекунд	   = 1;
	Если Не Отказ Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не Отказ Тогда
		...
		|	НовыеЦены.ХарактеристикаНоменклатуры,
		|	НовыеЦены.Цена,
		|	НовыеЦены.ТипЦен,
		|	НовыеЦены.Валюта
		|ПОМЕСТИТЬ НовыеЦены
		...
		|		ПО ЦеныНоменклатурыСрезПоследних.ТипЦен = НовыеЦены.ТипЦен
		|			И ЦеныНоменклатурыСрезПоследних.Номенклатура = НовыеЦены.Номенклатура
		|			И ЦеныНоменклатурыСрезПоследних.Цена <> НовыеЦены.Цена
		|ГДЕ
		|	НовыеЦены.ТипЦен <> &ТипЦен";
		Запрос.УстановитьПараметр("НовыеЦены", Товары.Выгрузить());
			Запрос.УстановитьПараметр("ТипЦен", Справочники.ТипыЦенНоменклатуры.ЦенаЗакупкиВес);
		ТЗ = Запрос.Выполнить().Выгрузить();
		
		Текст = "";
		Для Каждого Строка Из ТЗ Цикл 
			Текст = Текст + Строка.Номенклатура.Артикул + " - Нова ціна (" + Строка.ТипЦен + "): " + Строка.Цена + " " + Строка.Валюта + Символы.ПС; 
		КонецЦикла;
