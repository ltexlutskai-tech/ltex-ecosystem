...
Функция ПечатьСчетаЗаказа(Тип)
	...
	|	ВЫРАЗИТЬ(ЗапросСоставаТЧ.Товар КАК Строка(100)),
...
Функция ПечатьСчетаЗаказаКорректировки(Тип)
	...
	|
	|УПОРЯДОЧИТЬ ПО ВложенныйЗапрос.НомерТЧ ВОЗР, НомерСтроки ВОЗР
	|";
	...
	|	ВложенныйЗапрос.Цена,
	|	ВложенныйЗапрос.Сумма" + ТекстПоляАртикула + "
	|ИЗ
	|	(
	|	ВЫБРАТЬ
	|		ЗаказПоставщику.Номенклатура,
	|		ЗаказПоставщику.Цена                        КАК Цена,	
	|		СУММА(ЗаказПоставщику.Количество)           КАК Количество,
	|		СУММА(ЗаказПоставщику.Сумма     )           КАК Сумма" + ТекстВыборкиАртикула + "
	|	ИЗ
	|		Документ.ЗаказПоставщику.ВозвратнаяТара КАК ЗаказПоставщику" + ТекстИсточникАртикула + "
	|
	|	ГДЕ
	...
	|		КорректировкаЗаказаПоставщику.Цена                        КАК Цена,	
	|		СУММА(КорректировкаЗаказаПоставщику.Количество)           КАК Количество,
	|		СУММА(КорректировкаЗаказаПоставщику.Сумма     )           КАК Сумма" + ТекстВыборкиАртикула + "
	|	ИЗ
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = СуммаДокумента + НалоговыйУчет.ПолучитьСуммуДокументаСНДС(ЭтотОбъект, "Услуги");
