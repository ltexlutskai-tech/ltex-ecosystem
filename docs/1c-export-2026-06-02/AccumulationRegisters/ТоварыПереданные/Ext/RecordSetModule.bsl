...
Процедура КонтрольОстатков_ЗаказВШапке(ДокументОбъект)
	...
	ИначеЕсли мИмяДокумента = "КорректировкаДолгаПоВозвратнойТаре" Тогда	
		ИмяСуммыНДС = "Док.СуммаНДС";
		ИмяСуммы    = ?(ЕстьСуммаВключаетНДС И мСтруктураШапкиДокумента.СуммаВключаетНДС, "(" + ИмяСуммы + "-" + ИмяСуммыНДС + ")", ИмяСуммы);
	КонецЕсли;
