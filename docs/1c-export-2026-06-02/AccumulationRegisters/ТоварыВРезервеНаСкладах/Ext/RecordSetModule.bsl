...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	ПусатяХарактеристика = Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка();
	мТЗ = ЭтотОбъект.Выгрузить();
	Для Каждого Элемент из мТЗ Цикл 
		   Элемент.ХарактеристикаНоменклатуры = ПусатяХарактеристика;
	КонецЦикла;
	мТЗ.Свернуть("Период, Регистратор, Активность, ВидДвижения, Склад, Номенклатура, ХарактеристикаНоменклатуры, ДокументРезерва, СерияНоменклатуры", "Количество");
