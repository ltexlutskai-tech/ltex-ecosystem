...
Функция ОбработатьВведенныйШтрихкод(Штрихкод, Клиент) Экспорт
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ИначеЕсли ТипЗнч(РезультатЗапроса.Владелец) = Тип("ДокументСсылка.МаршрутныйЛист") Тогда
			Ответ = Клиент.СШКМаршрутныйЛист(РезультатЗапроса.Владелец, СШК);
			Если Ответ Тогда
				Возврат Результат;
			КонецЕсли;
...
Функция ВнестиСумму(Идентификатор, Пароль, Сумма) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		Если Сумма > 0 Тогда
			Обработка.ВывестиИнформациюНаДисплейПокупателя(Объект, Символы.ПС + "Внесено: " + Формат(Сумма, "ЧДЦ=2; ЧРГ=' '; ЧРД='.'; ЧН=0.00; ЧГ=0"), Сумма);
		Иначе
			ИзъятоНаДисплейПокупателя = НСтр("ru='Изъято: ';uk='Вилучено: '",КодЯзыка);
			Обработка.ВывестиИнформациюНаДисплейПокупателя(Объект, Символы.ПС + ИзъятоНаДисплейПокупателя + Формат(-Сумма, "ЧДЦ=2; ЧРГ=' '; ЧРД='.'; ЧН=0.00; ЧГ=0"), -Сумма);
		КонецЕсли;	
...
мCODE128                    = ПланыВидовХарактеристик.ТипыШтрихкодов.Code128;
