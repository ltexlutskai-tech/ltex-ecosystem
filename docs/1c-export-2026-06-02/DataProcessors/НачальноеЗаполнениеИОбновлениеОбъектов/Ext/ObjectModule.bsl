...
Функция ПолучитьПредопределенный(ТипРеквизита, Менеджер, ИмяПредопределенного)
	...
	//	СообщитьСлужебнуюИнформацию("Не найден предопределенный элемент "+ИмяПредопределенного+" для "+Менеджер);
	//КонецЕсли;
...
Процедура ЗаполнитьСправочник(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется справочник ';uk='Заповнюється довідник '")+ИмяОбъекта);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден предопределённый элемент %1 для справочника %2';uk='Не знайдений визначений елемент %1 для довідника %2'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		...
		Для СчК = 4 по ЧислоРеквизитов Цикл
			...
			Параметр = Макет.Область(Сч, СчК).ПараметрРасшифровки;
...
Процедура ЗаполнитьРегистрСведений(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется регистр сведений ';uk='Заповнюється регістр відомостей '")+ИмяОбъекта);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, МетаданныеРесурса));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Период = Макет.Область(Сч, 1).Текст;
		Если Найти(Период,":") = 0 Тогда
			//Добавим время
			Период = Период + " 0:00:00";
		КонецЕсли;	
		
		ТекущаяЗапись = РегистрыСведений[ИмяОбъекта].СоздатьМенеджерЗаписи();
		
		Если (НЕ МетаданныеРС.ПериодичностьРегистраСведений = Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический) 
			И НЕ ПустаяСтрока(Период) Тогда
		...
		Для СчК = 1 по ЧислоИзмерений Цикл
			...
			Параметр = ОбластьИзмерений.Область(Сч, СчК).ПараметрРасшифровки;
		...
		Для СчК = 1 по ЧислоРесурсов Цикл
			...
			Параметр = ОбластьРесурсов.Область(Сч, СчК).ПараметрРасшифровки;
...
Процедура ЗаполнитьТабличнуюЧасть(ТекущаяЗапись, Имя, ИмяТЧ, МакетТЧ, ТаблицаРеквизитовТЧ, МетаданныеТЧ, ФлагИзменений)
	...
	Для НомСтроки = 1 по ЧислоСтрок  Цикл
		...
		Для Сч = 1 по ЧислоРеквизитов Цикл
			...
			Параметр = ТекущаяОбласть.Область(НомСтроки, Сч).ПараметрРасшифровки;
...
Процедура ЗаполнитьПланВидовХарактеристик(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план видов характеристик ';uk='Заповнюється план видів характеристик '")+ИмяОбъекта);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден предопределённый элемент %1 для ПВХ %2';uk='Не знайдений визначений елемент %1 для ПВХ %2'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
...
Процедура ЗаполнитьПланСчетов(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план счетов ';uk='Заповнюється план рахунків '")+ИмяОбъекта);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден предопределённый элемент %1 для плана счетов %2';uk='Не знайдений визначений елемент %1 для плану рахунків %2'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
...
Процедура ЗаполнитьПланВидовРасчета(ИмяОбъекта)
	...
	СостояниеЗаполнения(НСтр("ru='Заполняется план видов расчета ';uk='Заповнюється план видів розрахунку '")+ИмяОбъекта);
...
"Об'єкт не заповнений!"""), ИмяОбъекта, ИмяРеквизита));
	...
	Для Сч = 2 По КоличествоЗаписей Цикл
		...
		Если СсылкаНаЭлемент = Неопределено Тогда
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не найден предопределённый элемент %1 для плана видов расчета %2';uk='Не знайдений визначений елемент %1 для плану видів розрахунку %2'"), Имя, ИмяОбъекта));
			Возврат;
		КонецЕсли;
		ТекущаяЗапись = СсылкаНаЭлемент.ПолучитьОбъект();
		...
		Если ЕстьТЧ Тогда
			...
			Для Каждого ТЧ из СписокТЧ Цикл
				...
				ЗаполнитьТабличнуюЧасть(ТекущаяЗапись, Имя, ТЧ.Значение, МакетТЧ, ТаблицаРеквизитовТЧ, МетаданныеПВР.ТабличныеЧасти[ТЧ.Значение], ФлагИзменений);	
