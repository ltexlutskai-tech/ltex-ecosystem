		...
		Если Строка.Представление = НСтр("ru='Р/счет';uk='Р/рахунок'") и  Строка.Пометка Тогда
			Попытка
				...
				Если Не ЗначениеЗаполнено(НовыйЭлемент.ОсновнойБанковскийСчет) Тогда
					НовыйСчет.Наименование    = НСтр("ru='Основной текущий счет';uk='Основний поточний рахунок'");
					НовыйСчет.Записать();
					НовыйЭлемент.ОсновнойБанковскийСчет =НовыйСчет.Ссылка;
				Иначе
					НовыйСчет.Наименование    = Лев(""+ НовыйБанк +НСтр("ru=', (Текущий)';uk=', (Поточний)'"),100);
					НовыйСчет.Записать();
				КонецЕсли;
...
Процедура СформироватьСтруктуруТаблицыИмпорта(Таб, ТабПоле) Экспорт
	...
	Т_ВидыОпераций  = Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОперацийППИсходящее,ПеречислениеСсылка.ВидыОперацийПоступлениеБезналичныхДенежныхСредств,ПеречислениеСсылка.ВидыОперацийСписаниеБезналичныхДенежныхСредств,ПеречислениеСсылка.ВидыОперацийПоступлениеБезналичныхДенежныхСредств,ПеречислениеСсылка.ВидыОперацийПоступлениеБезналичныхДенежныхСредств");
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "СчетОрганизации",      Т_БанкСчет,  НСтр("ru='Т/с организации';uk='П/р організації'"),            Видимая160);
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "СчетКонтрагента",      Т_БанкСчет,  НСтр("ru='Т/с контрагента';uk='П/р контрагента'"),            Видимая160);
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "ПлательщикСчет",       Т_СтрокаНомерСчета,  НСтр("ru='Расчетный счет получателя';uk='Розрахунковий рахунок отримувача'"),  НеВидимая);
...
Процедура СформироватьСтруктуруТаблицыРасчСчетов(Таб) Экспорт
	...
	СоздатьКолонкуВТаблице(Таб, Неопределено, "РасчСчетОрг",      Т_Строка,     НСтр("ru='Текущий счет организации';uk='Поточний рахунок організації'"), Видимая);
	...
	СоздатьКолонкуВТаблице(Таб, Неопределено, "РасчетныйСчет",    Т_Справочник, НСтр("ru='Текущий счет';uk='Поточний рахунок'"),            Видимая);
...
Функция ПолучитьТекстЗапросаПоПлатежноеПоручениеИсходящее()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	   ...
	   |	ПлатежноеПоручениеИсходящее.Организация.КодПоЕДРПОУ КАК ОрганизацияОКПО,
...
Функция ПолучитьТекстЗапросаПоАккредитивПереданный()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	   ...
	   |	АккредитивПереданный.Организация.КодПоЕДРПОУ КАК ОрганизацияОКПО,
	   ...
	   |	АккредитивПереданный.Контрагент.КодПоЕДРПОУ КАК КонтрагентОКПО,
...
Функция ПолучитьТекстЗапросаПоПлатежноеТребованиеВыставленное()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	   ...
	   |	ПлатежноеТребованиеВыставленное.Организация.КодПоЕДРПОУ КАК ОрганизацияОКПО,
	   ...
	   |	ПлатежноеТребованиеВыставленное.Контрагент.КодПоЕДРПОУ КАК КонтрагентОКПО,
...
Функция ПолучитьТекстЗапросаПоПлатежноеТребованиеПоручениеВыставленное()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	   ...
	   |	ПлатежноеТребованиеПоручениеВыставленное.Организация.КодПоЕДРПОУ КАК ОрганизацияОКПО,
	   ...
	   |	ПлатежноеТребованиеПоручениеВыставленное.Контрагент.КодПоЕДРПОУ КАК КонтрагентОКПО,
...
Функция ЗагрузитьСекциюРасчСчета(СтрокаРССчета)
	...
	Пока РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Цикл
		...
		Иначе
			// неправильный реквизит заголовка
			Сообщить(НСтр("ru='Неверный реквизит в секции описания текущего счета, строка ';uk='Невірний реквізит у секції опису поточного рахунку, рядок '") + (Импорт_ТекущаяСтрока - 1) + " : " + СтрокаРазбора);
			Возврат Ложь;
...
Функция ЗагрузитьСекциюРасчСчетаXML(СтрокаРССчета, ФайлXML)
	...
	Пока ФайлXML.Прочитать() Цикл
		...
		Иначе
			// неправильный реквизит заголовка
			Сообщить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный реквизит в секции описания текущего счета: %1';uk='Невірний реквізит в секції опису поточного рахунку: %1'"), ИмяТега));
			Возврат Ложь;
	...
	Если ВсеРеквизитыПоискаЕсть Тогда
		...
		Пока ВыборкаПоиска.Следующий() Цикл
			...
			Если ИщемПоПолномуНомеру Тогда // по полному соответствию номера
				Если СокрЛП(Формат(ВыборкаПоиска[РеквизитНомера],"ЧГ=")) = НомерДок Тогда
					Если КоличествоДок=0 Тогда
						СтрокаДокумента.Документ = ВыборкаПоиска.Ссылка;
						СтрокаДокумента.Проведен = ВыборкаПоиска.Проведен;
						СтрокаДокумента.НомерДок = Формат(ВыборкаПоиска[РеквизитНомера],"ЧГ=");
	...
	Пока Импорт_ТекущаяСтрока <= Импорт_КоличествоСтрок Цикл
		...
		ИначеЕсли Лев(Врег(СокрЛП(Стр)), 14)="СЕКЦИЯРАСЧСЧЕТ" Тогда
			...
			Если НЕ ЗагрузитьСекциюРасчСчета(СтрокаРССчетов) Тогда
				Сообщить(НСтр("ru='Нарушена структура файла импорта в секции описания текущего счета! Строка: ';uk='Порушено структуру файлу імпорту в секції опису поточного рахунку! Рядок: '") + (Импорт_ТекущаяСтрока - 1));
				Возврат;
			КонецЕсли;
...
//СуммаДокумента
	...
	Если ОбъектДокумента.ЭтоНовый() 
		...
		ЗначениеСтавкиНДС     = НалоговыйУчет.ПолучитьСтавкуНДС(СтрокаРасшифровки.СтавкаНДС);
