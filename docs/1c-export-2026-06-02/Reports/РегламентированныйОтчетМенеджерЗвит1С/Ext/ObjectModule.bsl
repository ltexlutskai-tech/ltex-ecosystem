	...
	Для сч = 1 По ОбластьСтруктура.ВысотаТаблицы Цикл
		...
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			...
			Если ТекОтчетМетаданные.Формы.Найти(ИмяФормы) = Неопределено Тогда
				Сообщить("FREDO Звіт. Заполнение соответствия отчетов <-> поддерживаемых схем Звит. В отчете """+ТекущийОтчет+""" ссылка на несуществующую форму отчета  """+ИмяФормы+""", которая заполненяет схему " + ИмяСхемыЗвит1С + "!", СтатусСообщения.Внимание);
			КонецЕсли;
...
Функция ЗаполнитьПараметрыРаботыЗвит1С()
	...
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить(-1, НСтр("ru = 'Не выгружен в ""FREDO Звіт""'; uk = 'Не вивантажен в ""FREDO Звіт""'"));
	ПредставлениеСостоянийДляИсходящихНалоговыхДокументов.Вставить( 0, НСтр("ru = 'Выгружен в ""FREDO Звіт""'; uk = 'Вивантажен в ""FREDO Звіт""'"));
	...
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить(-1, НСтр("ru = 'Не сформирован запрос в ""FREDO Звіт""'; uk = 'Не сформирован запит в ""FREDO Звіт""'")); // -1 Еще не реализовано со стороны FREDO Звіт, пока используется +2
	ПредставлениеСостоянийДляВходящихНалоговыхДокументов.Вставить( 1, НСтр("ru = 'Запрос сформирован в ""FREDO Звіт""'; uk = 'Запит сформирован в ""FREDO Звіт""'"));       //  0 Еще не реализовано со стороны FREDO Звіт, пока используется +2
...
Функция ПолучитьЗначениеПоказателя1С(Данные, ИмяТаблицы,ИмяПоказателя)
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Если ФлагОтладки ИЛИ ФлагОтладкиПакетноеТестирование Тогда
			Сообщить("FREDO Звіт. Ошибка получения показателя из 1С для передачи его в Звит (имя табличного поля с показателем """ + ИмяТаблицы + """, имя показателя 1С """ + ИмяПоказателя + """). Имя табличного поля или показателя указано не верно!", СтатусСообщения.Внимание);
		КонецЕсли;
	КонецПопытки;
...
Функция ЗапуситьЗвит1С(НаПереднийПлан = Неопределено, Организация = Неопределено) Экспорт
	...
	Если РезультатЗапуска = Неопределено Тогда
		Предупреждение(НСтр("ru = 'Запуск ""FREDO Звіт"" отменен. Возможные причины:
                             |
                             ...
                             |
                             | 1. Натиснута кнопка ""Відмініті"" у формі вибору робочого каталогу ""FREDO Звіт""
                             | 2. У каталозі з ""FREDO Звіт"" залишився файл ""zvit1c.ini"" (для запуску ""FREDO Звіт"" достатньо видалити даний файл)'"));	
		Возврат Ложь;
		
	ИначеЕсли РезультатЗапуска = 0 Тогда
		Предупреждение(НСтр("ru = 'Ошибка подключения к системе ""FREDO Звіт"". Возможные причины:
                             |
                             | 1. Нажата кнопка ""Відмінити"" в форме выбора рабочего каталога ""FREDO Звіт""
                             | 2. В каталоге с ""FREDO Звіт"" остался файл ""zvit1c.ini"" (для запуска ""FREDO Звіт"" достаточно удалить данный файл)'; uk = 'Помилка підключення до системи ""FREDO Звіт"". Можливі причини:
                             |
                             | 1. Натиснута кнопка ""Відмініті"" у формі вибору робочого каталогу ""FREDO Звіт""
                             | 2. У каталозі з ""FREDO Звіт"" залишився файл ""zvit1c.ini"" (для запуску ""FREDO Звіт"" достатньо видалити даний файл)'"));	
		Возврат Ложь;
...
Функция ВыгрузитьДокументВЗвит1С(ДокументСсылка, ПараметрыПакетнойВыгрузки = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Организация.ДатаНачалаИспользованияЗвит1С) Тогда
		ТекстОшибки = НСтр("ru = '""FREDO Звіт"" не используется для организации ""'; uk = '""FREDO Звіт"" не використовується для організації ""'")+Строка(Организация)+"""."+Символы.ПС+
		              НСтр("ru = 'В карточке организации не указана дата начала использования ""FREDO Звіт""'; uk = 'В картці організації не вказана дата початку використання ""FREDO Звіт""'");
		Возврат СформироватьРезультатВыгрузкиДокумента(ТекстОшибки, Истина);
	КонецЕсли;
