...
- **Фаза 0 — Фундамент:** `Product.receiptName` + поле в картці товару + скрипт переносу з 1С;
  резолвер загальної назви; клієнт НП API; конфіг env; структуровані НП-поля на Sale; **довідник/автопошук
  відділень НП у формі реалізації** (заміна вільного тексту), адреса для курʼєра.
- **Фаза 1 — ТТН при проведенні:** хук створення ТТН (орієнтовні місця/вага, накладка, цінність, авто-отримувач);
  збереження ref/номера; обробка помилок + «Повторити»; лінк на кабінет НП у завданні складу.
