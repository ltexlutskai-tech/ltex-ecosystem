...
## Блок 3 — товари/категорії/фото в системі (у процесі)

Рішення user: 1A (керує каталогом лише admin+owner+warehouse); лоти — лише через
Поступлення (ручне створення лотів НЕ додаємо).

- **3.1 Фото товару в CRM-картці ✅ (merged):** `lib/manager/catalog-permissions.ts`
...
## Блок 4 — дашборд сайту (зроблено)

- Лічильник візитів без кукі/PII: `SiteVisitDay`/`SiteVisitor` (міграція
  `20260710_site_visits`); `/api/track/visit` (хеш ip+ua+день+сіль);
  `VisitTracker` бекон у лейауті сайту (на кожен перехід).
- `lib/site-stats.getSiteStats`: візити/унікальні (сьогодні+7дн), замовлення з
  сайту (30дн), активні ліди, топ переглядів (ViewLog). Секція «Сайт» на
  дашборді адмінки.

## Довантаження після Блоку 3

- **Каскадний вибір категорій** (`CategoryCascader`) — рівень за рівнем, у 3
  місцях (батько категорії / створення товару / картка товару).
- **Приховані категорії** (`Category.hiddenFromCatalog`, міграція
  `20260708_category_hidden`) — фільтр на каталозі, пошуку, прайсі, **лотах**.
- **Картка товару розширена** (1С-поля): виробник (`Product.producer`, міграція
  `20260709_product_producer`), YouTube-посилання, авто-код у форматі 1С
  (MAX(code_1c)+1, 11 знаків), авто-дата.

## Статус: майстер-план 7.0 — ЗАВЕРШЕНО ✅

Блоки 0–4 усі зроблені. Хвіст (не блокує): приховані категорії не фільтруються
на головній (рейки featured/new — низький ризик); чистка не-1С товарів (Задача B)
чекає підтвердження цифр user.

## Усі міграції сесії 7.2 (для deploy)

`20260706_block1_site_orders`, `20260707_mgr_leads`, `20260708_category_hidden`,
`20260709_product_producer`, `20260710_site_visits` — усі застосовуються одним
