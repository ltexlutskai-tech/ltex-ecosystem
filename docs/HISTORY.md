...
## Session 51 — Search Facets (Price Slider + Multi-Select)

**Date:** 2026-04-28
**Branch:** `claude/session-51-search-facets` → merged at `0baa2c1`
**Spec:** `docs/SESSION_51_SEARCH_FACETS.md`

### Що зроблено

**Backend** (`apps/store`):

- `lib/catalog.ts` — `getCatalogProducts` + `fullTextSearch` приймають `quality | country` як `string | string[]`. Comma-separated значення з URL → `where.quality.in: [...]`. Backward compat: single value працює як раніше.
- `app/api/catalog/price-range/route.ts` (new) — `force-dynamic`, агрегує `floor(min)/ceil(max)` по `priceType: "wholesale"`, віддає `Cache-Control: max-age=300`. 3 vitest cases.
- `lib/catalog.test.ts` (+58) — 5 тестів для multi-select parsing.

**Web** (`apps/store/components/store`):

- `catalog-filters.tsx` — quality + country переписані на checkbox-групи (URL — comma-separated). Price slider замість 2 input + OK button.
- `price-range-slider.tsx` (new) — dual-handle через 2 overlapped `<input type="range">`, `onCommit` на mouseup/touchend.
- `price-range-slider.test.tsx` (new) — 4 tests.

**Mobile** (`apps/mobile-client`):

- `CatalogFilters` interface: `qualities: string[]`, `countries: string[]`.
- `CatalogFilterSheet.tsx` — Ionicons checkbox lists для quality + country, інтеграція слайдера, fetch bounds одноразово через `catalogApi.priceRange()`.
- `PriceRangeSlider.tsx` (new) — pure-RN з 2 PanResponders, без додаткових deps.
- `CatalogScreen.tsx` — `qualities.join(",")` / `countries.join(",")` перед `/api/catalog`.

### Verification

Worker: format ✅, typecheck ✅, tests 294/294 (269 store + 25 shared, 283 baseline + 11), `next build` clean. Orchestrator merge clean fast-forward.

### Файли (11 changed, +868/-208)

- `apps/store/lib/catalog.ts` (+77/-? — multi-select parsing)
- `apps/store/lib/catalog.test.ts` (new, 58)
- `apps/store/app/api/catalog/price-range/route.ts` (new, 25)
- `apps/store/app/api/catalog/price-range/route.test.ts` (new, 61)
- `apps/store/components/store/catalog-filters.tsx` (+233/-? — checkboxes + slider)
- `apps/store/components/store/price-range-slider.tsx` (new, 125)
- `apps/store/components/store/price-range-slider.test.tsx` (new, 84)
- `apps/mobile-client/src/lib/api.ts` (+4 — `priceRange()`)
- `apps/mobile-client/src/components/PriceRangeSlider.tsx` (new, 166)
- `apps/mobile-client/src/components/CatalogFilterSheet.tsx` (+237/-? — checkboxes + slider)
- `apps/mobile-client/src/screens/catalog/CatalogScreen.tsx` (+6 — comma-join)
...
- Server-side facet counts ("Англія (45)")
