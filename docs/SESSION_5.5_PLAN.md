...
#### Деталізований план звіту (на затвердження user-ом)

**5.5-Звіт-0 — помашинні рухи боргу (фундамент для старіння).** Зараз
`importDebt` пише ОДИН `opening`-агрегат на клієнта. Для старіння треба дати
документів. → Розширити `importDebt`: читати `_AccumRg5269` з `_Period` (дата
руху) + `_RecorderRRef` (документ) + `_RecordKind` + клієнт + сума → писати
ОКРЕМИЙ `MgrDebtMovement` на рядок реєстру (`occurredAt=_Period`,
`sourceType="accum_rg5269"`, `sourceId="{recorderHex}:{lineNo}"` — унікально,
ідемпотентно; `kind`: приход→`opening` дебет / расход→`payment`). Σ незмінна =
тотал боргу. **Не подвоює** live-рухи (історичні = до cutover).
⚠️ Обсяг: 78328 рухів (vs 2858 агрегатів) — більше рядків, але разовий імпорт.

**5.5-Звіт-1 — назви папок-груп.** Доімпортувати назви папок контрагентів
(`_Reference***` де `_Folder`=група) як мапу hex→назва → нове поле
`MgrClient.parentName` (або окремий довідник). Для групування Покупці/Бузна/
Проблемна дебіторка.

**5.5-Звіт-2 — обчислення колонок (хелпер `lib/reports/overdue-debts.ts`):**

- **Борг** = `MgrClient.debt` (Σ рухів, EUR). ✓ є.
- **Прострочений борг** = Σ рухів `occurredAt < today − поріг_днів` (нетто за
  датою — апроксимація старіння; не повна FIFO-алокація, але близько до 1С).
- **Борг по наложці** = Σ боргу по реалізаціях з `cashOnDelivery=true`. Резолв:
  рух `sourceType=accum_rg5269` → `_RecorderRRef`→`Sale` (за `code1C`) →
  `cashOnDelivery`. (Якщо реалізацію не зрезолвлено — у наложку не йде.)
- **Днів відвантаження** = `daysSinceLastPurchase`. ✓ є.
- **Діяльність** = «Претензійна робота!» коли прострочений борг > 0. ✓ обчисл.
- **Торговий агент** = `MgrClient.agent.fullName` (або `agentUserId`). ✓ є.

**5.5-Звіт-3 — UI + експорт.** Апгрейд `/manager/reports/debts`: групування по
папці (parentName), колонки за 1С-формою, поріг днів (дефолт 14, керований),
червоне підсвічування протермінованих, **усі суми EUR**, CSV-експорт (наявний
патерн `analyst-reports` + `/csv`). Доступ — analyst/owner/bookkeeper/admin
(як інші звіти).

**Компроміси (на підтвердження):** старіння — нетто-за-датою (не FIFO); наложка —
лише по зрезолвлених реалізаціях; групи — по папці-батьку (плоско, без глибокої
ієрархії L-TEX SECOND→підгрупи). Уточнюється після першого прогону на реальних
даних.
