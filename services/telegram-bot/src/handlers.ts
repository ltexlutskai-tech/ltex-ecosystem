...
async function handleOrder(chatId: number, orderId: string): Promise<void> {
  ...
  const itemLines = order.items.slice(0, 5).map(
    (item) => `  • ${escapeHtml(productNames.get(item.productId) ?? "?")} — ${item.weight} кг, €${item.priceEur.toFixed(2)}`,
  );
  ...
  const text = [
    ...
    `<b>Сума:</b> €${order.totalEur.toFixed(2)}`,
    order.totalUah > 0 ? `<b>Сума (UAH):</b> ${order.totalUah.toFixed(2)} ₴` : "",
    `<b>Позицій:</b> ${order.items.length}`,
    `<b>Дата:</b> ${new Date(order.createdAt).toLocaleDateString("uk-UA")}`,
    ``,
    `<b>Товари:</b>`,
    ...itemLines,
  ]
