...
/**
 * Друк-сторінка реалізації — «Видаткова накладна» (← 5.4.4, перший пас).
 *
 * Аналог 1С форми друку «Реалізація товарів і послуг»: табличний документ
 * A4 з шапкою (постачальник / покупець), рядками
 * (№ / Артикул / Товар / Мішків / Вага / Ціна за кг / Сума), підсумками
 * (сума EUR + грн, курс, наложка, відділення НП, примітка) і місцем для
 * підписів «Відвантажив / Отримав».
 *
...
export default async function PrintSalePage({
  ...
  // Адреса покупця (з картки клієнта за code1C, якщо є).
  const mgr = sale.customer.code1C
  ...
  const buyerStreet = mgr
    ...
    : "";
  const buyerLocality = [sale.customer.city, buyerStreet]
    .filter(Boolean)
    .join(", ");

  const totalQuantity = sale.items.reduce((s, i) => s + i.quantity, 0);
  ...
  return (
    <div className="print-page">
      ...
      <div className="toolbar">
        ...
        <a
          href={`/manager/sales/${id}`}
          style={{ color: "#374151", textDecoration: "none", fontSize: 14 }}
        >
          ← Назад до документа
        </a>
        <span style={{ marginLeft: "auto", color: "#6b7280", fontSize: 13 }}>
          № {displayNumber} · {formatDate(sale.createdAt)}
        </span>
      </div>

      <div className="sheet">
        <h1>
          Видаткова накладна № {displayNumber} від {formatDate(sale.createdAt)}
        </h1>
        ...
        <div className="parties">
          ...
          <div className="party">
            ...
            {sale.customer.phone && <div>Тел.: {sale.customer.phone}</div>}
          </div>
        </div>
          ...
          <tbody>
            {sale.items.map((it, idx) => (
              <tr key={it.id}>
                ...
                <td className="num">{money2(it.pricePerKg)}</td>
        ...
        <div className="summary">
          <div>
            <strong>
              Всього на суму: {money2(sale.totalEur)} € (
              {Math.round(sale.totalUah).toLocaleString("uk-UA")} ₴)
            </strong>
          </div>
          {sale.exchangeRateEur > 0 && (
            <div>Курс: {money2(sale.exchangeRateEur)} ₴/€</div>
          )}
          {sale.cashOnDelivery && sale.codAmountUah != null && (
            <div>
              Накладений платіж:{" "}
              {Math.round(sale.codAmountUah).toLocaleString("uk-UA")} ₴
            </div>
          )}
          {sale.novaPoshtaBranch && (
            <div>Нова Пошта, відділення: {sale.novaPoshtaBranch}</div>
          )}
          {sale.notes && <div>Примітка: {sale.notes}</div>}
        </div>
