...
/**
 * Дії зміни статусу для read-only режиму перегляду реалізації (наприклад
 * `cancelled` → повернути в чернетку). PATCH вимагає items — передаємо
 * наявні через preloaded snapshot, щоб не дублювати товари.
 */
export function SaleStatusActions({
  ...
  const allowed = getAllowedSaleTransitions(status);
  if (allowed.length === 0) return null;

  async function changeStatus(next: ManagerSaleStatus): Promise<void> {
    setSubmitting(true);
    setError(null);
    try {
