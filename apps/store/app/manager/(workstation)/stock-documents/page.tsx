    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Повернення, перепаковка, списання, оприбуткування, інвентаризація,
          переміщення між складами (перенесено з 1С).
        </p>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {STOCK_DOCS.map((d) => (
          <Link
            key={d.kind}
            href={`/manager/stock-documents/${d.slug}`}
            className="block rounded-md border bg-white p-4 transition hover:border-emerald-400 hover:shadow-sm"
          >
