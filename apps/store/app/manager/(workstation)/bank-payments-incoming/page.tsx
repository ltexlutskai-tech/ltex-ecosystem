...
export const metadata = { title: "Платіжні доручення вхідні — L-TEX Manager" };

const PAGE_SIZE = 30;

export default async function BankPaymentsIncomingPage({
  searchParams,
}: {
  ...
  const [items, total] = await Promise.all([
    ...
    prisma.bankPaymentIncoming.count({ where }),
  ]);
  ...
  return (
    <div className="max-w-none space-y-3">
      <header className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-800">
            Платіжні доручення вхідні
          </h1>
          <p className="mt-1 text-sm text-gray-600">
            Безготівкові надходження від контрагентів · усього: {total}
          </p>
        </div>
        <Link
          href="/manager/bank-payments-incoming/new"
        ...
        <Link
          href={
            showArchived
              ...
              : "/manager/bank-payments-incoming?archived=1"
          }
      ...
      {items.length === 0 ? (
        <EmptyState
          message="Вхідних платіжок немає"
                ...
                {items.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2">
                      <Link
                        href={`/manager/bank-payments-incoming/${d.id}`}
                        className="text-emerald-700 hover:underline"
                      >
