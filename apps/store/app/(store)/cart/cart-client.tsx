...
export function CartClient({ rate }: CartClientProps) {
  ...
  const totalUah = formatUah(eurToUah(totalEur, rate));
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {items.map((item) => {
                  ...
                  const lineUah = formatUah(eurToUah(item.priceEur, rate));
                  const isApprox = !item.barcode;
                  return (
                    <tr key={key} className="border-b">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {isApprox && (
                          <span
                            title={dict.cart.approxPriceTooltip}
                            className="cursor-help"
                            aria-label={dict.cart.approxPriceTooltip}
                          >
                            ≈{" "}
                          </span>
                        )}
                        <strong>{lineUah}</strong>
