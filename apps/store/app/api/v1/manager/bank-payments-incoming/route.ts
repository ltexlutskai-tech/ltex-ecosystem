...
/**
 * Задача D — Платіжки вхідні (надходження безналу від клієнта). Список з
...
export async function GET(req: NextRequest) {
  ...
  const where: Prisma.BankPaymentIncomingWhereInput = {};
...
/** Створення чернетки вхідної платіжки. `amountEur` зводиться на сервері. */
export async function POST(req: NextRequest) {
  ...
  try {
    const created = await prisma.bankPaymentIncoming.create({
      data: {
    ...
    console.error("[L-TEX] BankPaymentIncoming create failed", {
      error: err instanceof Error ? err.message : String(err),
    });
    return NextResponse.json(
