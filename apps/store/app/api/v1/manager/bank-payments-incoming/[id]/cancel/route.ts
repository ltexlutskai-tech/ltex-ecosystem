...
import { cancelBankPaymentIncoming } from "@/lib/manager/treasury-posting";
import { treasuryActionResponse } from "@/lib/manager/treasury-action-response";

/** Скасування проведення вхідної платіжки (posted→cancelled). */
export async function POST(
  ...
  const result = await cancelBankPaymentIncoming(id);
