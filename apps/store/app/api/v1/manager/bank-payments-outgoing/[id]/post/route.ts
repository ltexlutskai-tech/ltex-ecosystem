...
import { postBankPaymentOutgoing } from "@/lib/manager/treasury-posting";
import { treasuryActionResponse } from "@/lib/manager/treasury-action-response";

/** Проведення вихідної платіжки (draft→posted): рух ДДС (розхід). */
export async function POST(
  ...
  const result = await postBankPaymentOutgoing(id);
