 ...
 *  - Оприбуткування / Повернення від покупця                               → прихід
 *  - Переміщення: відправник → розхід, одержувач → прихід
 *  - Перепаковка: розбирані → розхід, скомплектовані → прихід
 *  - Інвентаризація: рухів НЕ пише (лише звірка облік↔факт). Коригування
 *    залишків роблять ОКРЕМІ документи «на підставі інвентаризації» —
 *    Списання (нестача) та Оприбуткування (надлишок).
 *
  ...
  for (const item of doc.items) {
    ...
    switch (kind) {
      ...
      case "repackings":
        ...
        break;
      case "inventories":
        // Інвентаризація — документ звірки (облік↔факт), сама залишків НЕ рухає.
        // Нестачу списують, надлишок оприбутковують окремими документами
        // «на підставі інвентаризації». Тож тут рухів не пишемо.
        break;
