...
const { applyDebtMovementSafeMock } = vi.hoisted(() => ({ applyDebtMovementSafeMock: vi.fn() }));
...
import { nextDocNumber, docNumberPrefix, summarizeLines, getStockDocMeta, STOCK_DOCS, applyReturnFromCustomerDebt } from "./stock-documents";
...
describe("nextDocNumber", () => {
  ...
  it("returns max+1 padded", () => {
    expect(nextDocNumber("LT-RET-202606-", ["LT-RET-202606-0001", "LT-RET-202606-0007", "LT-RET-202606-0003"])).toBe("LT-RET-202606-0008");
  });
  it("ignores other prefixes", () => {
    expect(nextDocNumber("LT-RET-202606-", ["LT-WOF-202606-0009"])).toBe("LT-RET-202606-0001");
  });
});
...
describe("getStockDocMeta", () => {
  it("returns meta for each of 8 kinds", () => {
    ...
    for (const d of STOCK_DOCS) expect(getStockDocMeta(d.kind).label).toBe(d.label);
  });
...
describe("normalizeLine", () => {
  it("computes amountEur from weight × priceEur", () => {
    expect(normalizeLine({ weight: 20, priceEur: 1.5, quantity: 1 }).amountEur).toBe(30);
  });
  it("falls back to quantity × priceEur when weight 0", () => {
    expect(normalizeLine({ weight: 0, quantity: 4, priceEur: 2 }).amountEur).toBe(8);
  });
...
describe("summarizeLines", () => {
  it("sums weight, quantity and amountEur with rounding", () => {
    const lines: NormLine[] = [
      { productId: null, charHex: null, barcode: null, weight: 10.1, quantity: 2, priceEur: 0, amountEur: 15.005, notes: null },
      { productId: null, charHex: null, barcode: null, weight: 5.05, quantity: 1, priceEur: 0, amountEur: 4.5, notes: null },
    ];
  ...
  it("returns zeros for empty list", () => {
    expect(summarizeLines([])).toEqual({ totalWeight: 0, totalQuantity: 0, totalEur: 0 });
  });
});

describe("applyReturnFromCustomerDebt", () => {
  it("writes a negative correction movement for the customer", () => {
    applyReturnFromCustomerDebt({ returnId: "ret-1", customerId: "cust-1", totalEur: 42.5, occurredAt: new Date("2026-06-17"), createdByUserId: "user-1" });
  ...
  it("skips when no customer", () => {
    applyReturnFromCustomerDebt({ returnId: "ret-2", customerId: null, totalEur: 10, occurredAt: new Date() });
    expect(applyDebtMovementSafeMock).not.toHaveBeenCalled();
  });
  it("skips when total is zero", () => {
    applyReturnFromCustomerDebt({ returnId: "ret-3", customerId: "cust-3", totalEur: 0, occurredAt: new Date() });
    expect(applyDebtMovementSafeMock).not.toHaveBeenCalled();
  });
});
