 ...
 *
 * Канонічні статуси менеджерського замовлення (модель 7.3, як у 1С
 * «Заказ покупателя»):
 *   - `draft`     — Чернетка (редагується вільно);
 *   - `posted`    — Проведено: документ зафіксовано, редагування заблоковано,
 *                   замовлення ЛИШАЄТЬСЯ актуальним/видимим — воно в роботі
 *                   (Потреби, маршрути), допоки не закрите/скасоване/
 *                   відвантажене;
 *   - `cancelled` — Скасовано (⇒ archived + неактуальне).
 *
 * Незалежно від статусу:
 *   - `isActual` («Актуальне») — замовлення в роботі; правило «одне актуальне
 *     на клієнта». Знімається при закритті/скасуванні/відвантаженні маршрутом
 *     або вручну.
 *   - `archived` — прибрано з активного списку (скасовані + історія з 1С).
 *
 * Legacy-статуси (`sent`/`pending`/`approved`/`shipped`/`delivered`)
 * лишаються у META-мапі для коректного відображення старих замовлень, але не
 * пропонуються у нових документах.
 */
