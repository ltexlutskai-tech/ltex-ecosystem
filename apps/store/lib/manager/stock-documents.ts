...
export const STOCK_DOCS: readonly StockDocMeta[] = [
  {
    kind: "product-returns",
    slug: "product-returns",
    label: "Повернення від покупця",
    legacyName: "ВозвратОтПокупателя",
    numberPrefix: "RET",
    description:
      "Клієнт повертає товар. Прямо коригує борг (мінус-реалізація).",
  },
  {
    kind: "warehouse-returns",
    slug: "warehouse-returns",
  ...
  },
  {
    ...
    legacyName: "ВозвратТоваровПоставщику",
    numberPrefix: "SRT",
  ...
  },
  {
    ...
    legacyName: "ОприходованиеТоваров",
    numberPrefix: "ADJ",
  ...
  },
  {
    ...
    legacyName: "ИнвентаризацияТоваровНаСкладе",
    numberPrefix: "INV",
  ...
  },
  {
    ...
    legacyName: "ПеремещениеТоваров",
    numberPrefix: "TRF",
    description: "Переміщення товару склад-відправник → склад-одержувач.",
  },
] as const;
...
async function findExistingNumbers(
  db: PrismaClient,
  kind: StockDocKind,
  prefix: string,
): Promise<string[]> {
  ...
  switch (kind) {
    case "product-returns":
      return (
        await db.productReturnFromCustomer.findMany({ where, select })
      ).map((r) => r.docNumber);
    case "warehouse-returns":
      return (await db.warehouseReturn.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "supplier-returns":
      return (await db.returnToSupplier.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "repackings":
      return (await db.repacking.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "write-offs":
      return (await db.writeOff.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "stock-adjustments":
      return (await db.stockAdjustment.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "inventories":
      return (await db.inventory.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
    case "stock-transfers":
      return (await db.stockTransfer.findMany({ where, select })).map(
        (r) => r.docNumber,
      );
  }
}

export async function generateStockDocNumber(
  kind: StockDocKind,
  date: Date = new Date(),
...
export function summarizeLines(lines: readonly DocLineLike[]): {
  totalWeight: number;
