...
### Tasks for next session (Session 12)

**IMPORTANT:** НЕ повторювати seed, merge, або infrastructure setup — все вже зроблено.
**IMPORTANT:** НЕ повторювати задачі Session 4-11 — ВСЕ ЗРОБЛЕНО. Дивись completion reports вище.
...
#### Task: Надати покрокові інструкції для інфраструктури

Це НЕ кодинг-сесія. Worker має прочитати всі існуючі скрипти, конфіги, CLAUDE.md і написати ОДИН детальний покроковий гайд українською мовою, який користувач зможе виконати самостійно.

Гайд має бути в форматі Markdown і виведений у відповідь (НЕ записаний у файл).

**Покрити такі кроки (в правильному порядку):**

1. **Netlify deploy** — retry deploy, перевірити що працює, URL для перевірки `/api/health`
2. **Supabase RLS** — скопіювати з `scripts/enable-rls.sql`, вставити в SQL Editor, запустити
3. **Supabase FTS міграція** — скопіювати з `scripts/fts-migration.sql`, вставити в SQL Editor, запустити
4. **Netlify env vars** — які змінні додати, де їх взяти, як згенерувати SYNC_API_KEY
5. **Telegram бот** (опціонально) — створити бота, отримати токен, зареєструвати webhook
6. **Viber бот** (опціонально) — створити бота, отримати токен, зареєструвати webhook
7. **Перевірка** — які URL відкрити, що має працювати

**Для кожного кроку вказати:**

- Де саме натискати (Supabase Dashboard → SQL Editor → New query)
- Точні команди (з placeholder для секретів)
- Що має бути результатом (success message, що побачите)
- Можливі помилки та як їх виправити

**НЕ включати** в гайд:

- seed (вже запущений — 805 products, 725 lots в БД)
- prisma db push (вже зроблено)
- завантаження фото (поки немає фото)
- mobile app setup (окремо)
- 1С інтеграція (потрібна конфігурація на стороні 1С)
- кастомний домен (окремо)
